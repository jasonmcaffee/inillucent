# task-1761: rust-db scoring and retrieval evaluation
...
- Declaring a new retrieval method superior from published numbers alone. External research identifies candidates; rust-db's own frozen evaluation decides what ships.
- Replacing the current pgvector comparison. It answers a useful product question and should remain.
- Turning rust-db into a distributed database, transactional store, or general SQL engine.
...
| Dense retrieval | Retrieval using one or more learned numeric vectors. Semantically similar text can match without sharing exact words. rust-db currently uses one 768-dimensional vector per chunk. |
| Sparse or lexical retrieval | Retrieval through terms in an inverted index. It is strong for exact identifiers and shared vocabulary. rust-db currently uses BM25 plus coverage and proximity adjustments. |
...
| HNSW | Hierarchical Navigable Small World, the in-memory graph index used by rust-db and pgvector for approximate nearest-neighbor search. |
| Exhaustive search | Exact comparison of the query vector with every passing vector. It is the vector-recall ground truth and is often the fastest path for selective filters at rust-db's current scale. |
| Recall at k | The share of known relevant results found in the first `k` results. For approximate-vector testing, rust-db compares against the exact top `k`; for RAG, evidence recall asks how much required evidence was retrieved. |
...
| Score fusion | A weighted combination of normalized vector and lexical scores. rust-db currently defaults to per-list min-max scaling with vector weight 0.35. |
...
flowchart TB
    ...
    subgraph Evaluation ["Evaluation-time scoring"]
        QS["title, heading, identifier queries"] --> EN["rust-db and pgvector runs"]
...
The current implementation is spread across [`bm25.rs`](../crates/rustdb-core/src/bm25.rs), [`rank.rs`](../crates/rustdb-core/src/rank.rs), [`index.rs`](../crates/rustdb-core/src/index.rs), and the vector-search modules.
...
- The lexical and vector lists can be fused with the same policy in both rust-db and pgvector, keeping the comparison fair.
...
|---|---|---|
| [`store.rs`](../crates/rustdb-core/src/store.rs) | Stores documents, chunks, text, and dictionary-encoded metadata columns. | Gives filters a compact integer representation and maps chunk hits back to their documents. |
| [`filter.rs`](../crates/rustdb-core/src/filter.rs) | Represents source, author, label, date, and deletion predicates; compiles them for repeated checks. | Defines the authorized candidate set and exposes passing counts used by the exact-versus-graph cost model. |
| [`vectors.rs`](../crates/rustdb-core/src/vectors.rs) and [`distance.rs`](../crates/rustdb-core/src/distance.rs) | Hold normalized vectors contiguously and compute dot-product cosine distance. | Provide the exact numeric similarity used by both flat and graph retrieval. |
| [`flat.rs`](../crates/rustdb-core/src/flat.rs) | Scans every passing vector and reduces candidates to top `k`. | Supplies both an exact production path and the oracle for approximate-vector recall. |
| [`hnsw.rs`](../crates/rustdb-core/src/hnsw.rs) | Builds and walks the approximate graph while expanding rejected nodes but admitting only filter-passing nodes. | Trades distance computations for speed without allowing the predicate to disconnect traversal. |
| [`quantize.rs`](../crates/rustdb-core/src/quantize.rs) | Builds int8 codes and rescoring candidates against full-precision vectors. | Reduces first-pass memory bandwidth while preserving a full-precision final distance. |
| [`tokenize.rs`](../crates/rustdb-core/src/tokenize.rs) | English stemming, stopword handling, and preservation/splitting of identifier-like terms. | Controls lexical vocabulary, exact-token behavior, and language support. |
| [`bm25.rs`](../crates/rustdb-core/src/bm25.rs) | Inverted index, term positions, BM25, coverage, proximity, prefix, and tiering. | Produces the lexical candidate list and its raw relevance scores. |
| [`rank.rs`](../crates/rustdb-core/src/rank.rs) | RRF and score fusion, origin tracking, deterministic sorting, and document cap. | Produces the final hybrid order. |
| [`embed.rs`](../crates/rustdb-core/src/embed.rs) and [`embed_onnx.rs`](../crates/rustdb-core/src/embed_onnx.rs) | Define the embedding boundary and run Nomic in-process through ONNX Runtime when enabled. | Apply distinct document/query prefixes, dimensionality, pooling, and normalization. |
| [`persist.rs`](../crates/rustdb-core/src/persist.rs) | Saves and reopens versioned store, vector, graph, and configuration data. | Makes the embedded index restartable without a database server; lexical postings and codes are deterministically rebuilt on open. |
| [`index.rs`](../crates/rustdb-core/src/index.rs) | Composes all stages behind the public `Index` API. | Owns defaults, exact/graph routing, candidates, fusion, and final output. |
...
The current benchmark is implemented by [`queryset.rs`](../crates/rustdb-bench/src/queryset.rs), [`scenarios.rs`](../crates/rustdb-bench/src/scenarios.rs), [`metrics.rs`](../crates/rustdb-bench/src/metrics.rs), [`report.rs`](../crates/rustdb-bench/src/report.rs), and [`tune.rs`](../crates/rustdb-bench/src/tune.rs).

It builds 185,078 chunks from public material representing six workplace sources, embeds them once with `nomic-embed-text-v1.5`, and gives identical vectors to rust-db and PostgreSQL. It currently grades:
...
| Warm latency | unfiltered rust-db median `0.6061 ms`; filtered Slack median `0.5651 ms`. |
...
|---|---|
| rust-db enforces its filters | Strong. Every returned row is checked, including an absent filter value. |
...
| Filter-aware rust-db retrieval avoids pgvector's post-filter candidate loss | Strong for the source filters and pgvector configurations tested. |
...
| Latency mixes algorithm and deployment effects | In-process rust-db versus loopback PostgreSQL is a valid product comparison but not a pure index-speed comparison. Both views should be shown separately. | Medium |
...
|---|---|---|---|
| 0. Current parity card | Did rust-db preserve its exact-vector, filter, pgvector-parity, and performance properties? | Minutes | Required regression lane; unchanged in purpose. |
...
The hybrid-fusion experiment is particularly justified. The existing code cites and partially implements the family studied by [Bruch, Gai, and Ingber](https://arxiv.org/abs/2210.11934), who found convex combinations can outperform RRF and can be tuned sample-efficiently. rust-db should extend that work to query-adaptive weights only after establishing a proper train/validation/test split.
...
Dense retrieval remains important: [DPR](https://aclanthology.org/2020.emnlp-main.550/) established that learned dense passage retrieval can outperform strong BM25 on open-domain QA. The lesson is not to replace lexical search, because rust-db's identifiers prove its value; it is to test complementary representations under the actual query mix.
...
- Run current rust-db, pgvector defaults, correctly configured pgvector, plain BM25, dense exact, dense HNSW, and current hybrid on all retrieval packs.
- Separate candidate recall, final ranking, and deployed-system latency.
- Publish slice-level error analysis.

Acceptance: the report states where current rust-db is strong, weak, or inconclusive without relying on win counts alone.
