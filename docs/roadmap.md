...
**42.40 MiB against SQLite's 37.20 — 14% more**, on the same 128 MiB budget, while running 330%
...
| `read.join` | 332% faster (4.32x), lower bound **3.00x** | 200% faster — the lower bound sits exactly on the bar |
...
`read.join`'s weighted geometric mean — the family missing its bar on the 95% lower bound alone.
...
of each inner stage, with the borrows re-acquired per execution — and the same slot on the write
...
It was worth nothing measurable. Measured as a genuine A/B — the same fixture and command, the change
alternated in and out of the tree with a release rebuild each time, at two round counts — the paired
...
whole `extension` family under the performance contract's 1.00x floor — the bar that fails a release
...
|---|---:|---:|
| before the segment format | **1.32x** | — |
...
`inillucent-legacy` (660) — 30,697 lines, the engine that reached SQLite file format parity, its
connection, its facade and its `sqlite3_*` C ABI — are gone from the workspace. It was measured
...
**36 files in `inillucent-compat` named one of the four, not the 30 first estimated here** — the
...
- **re-pointed at the pinned SQLite oracle** — the majority, once `src/differential.rs` itself was
...
- **`tests/capi.rs` deleted rather than re-pointed** — it proved `inillucent-capi`'s ABI by compiling a
...
`new_engine_search.rs` did — no old store to compare against — and is now an absolute report rather
...
- **Four ways a checkpoint lost a database that the crash had left intact.** The crash
  ...
  engine during this ticket, and the two they had never been run against — `PRAGMA journal_mode =
  ...
  **The two meta pages were the only pages a checkpoint overwrote without a pre-image.** With the
  ...
  read `generation 5, checkpoint_lsn 17160` — a checkpoint that had not finished. Redo believed it,
  ...
  *checkpoint* is moving pages out of the log and into the data file. The commit campaigns commit
  ...
  were found — while `PRAGMA journal_mode = delete` matches the default, returns without doing
  ...
  *previous* checkpoint, and the log still held every record above it. Recovery failed because the
  ...
  — which holds the pre-images for the duration of a checkpoint and removes the file once the
  ...
  `truncate` and `persist` arms, which is the level that is allowed not to reach the media — and what
