...
`delta` is rust-db minus the baseline, oriented so positive is better whatever the metric's own direction. The interval is the 95% paired bootstrap on that delta; `p` is the paired randomization test. `n` is the queries behind it and `moved` is how many of them the two engines answered differently — a comparison resting on three queries is worth reading with suspicion however small its p-value.

| family | measurement | metric | rust-db | baseline | delta | 95% interval | p | n | moved | threshold | verdict |
|---|---|---|---|---|---|---|---|---|---|---|---|
| Filtered vector search, per source | source = confluence (93813 chunks, rust-db path: graph) | recall@10 within the filter | 0.9960 | 0.9760 | 0.0200 | 0.0000 to 0.0440 | 0.2529 | 25 | 3 | 0.0100 | inconclusive |
| Filtered vector search, per source | source = github (47497 chunks, rust-db path: graph) | recall@10 within the filter | 1.000 | 0.3320 | 0.6680 | 0.5280 to 0.7960 | 0.0005 | 25 | 24 | 0.0100 | better |
| Filtered vector search, per source | source = slack (17616 chunks, rust-db path: exhaustive) | recall@10 within the filter | 1.000 | 0.6120 | 0.3880 | 0.3120 to 0.4640 | 0.0005 | 25 | 25 | 0.0100 | better |
| Filtered vector search, per source | source = jira (9429 chunks, rust-db path: exhaustive) | recall@10 within the filter | 1.000 | 0.3280 | 0.6720 | 0.5360 to 0.7960 | 0.0005 | 25 | 22 | 0.0100 | better |
| Filtered vector search, per source | source = figma (9144 chunks, rust-db path: exhaustive) | recall@10 within the filter | 1.000 | 1.000 | 0.0000 | 0.0000 to 0.0000 | 1.0000 | 25 | 0 | 0.0100 | equivalent, at the ceiling |
| Filtered vector search, per source | source = miro (7396 chunks, rust-db path: exhaustive) | recall@10 within the filter | 1.000 | 0.8840 | 0.1160 | 0.0520 to 0.1920 | 0.0020 | 25 | 11 | 0.0100 | better |
...
| source = confluence (93813 chunks, rust-db path: graph) | recall@10 within the filter | **0.9960** | 0.8160 | 0.9760 |
| source = github (47497 chunks, rust-db path: graph) | rows returned of 50 requested | **50.000** | 1.360 | 34.960 |
| source = github (47497 chunks, rust-db path: graph) | recall@10 within the filter | **1.000** | 0.0680 | 0.3320 |
...
| source = miro (7396 chunks, rust-db path: exhaustive) | recall@10 within the filter | **1.000** | 0.0640 | 0.8840 |
...
|---|---|---|---|---|---|---|---|---|---|
| rust-db | 185078 | 38847 | 165.2 | 4 | 6119440 | 494179 | 11643704 | 568.6 | 142.9 |
...
| field | value |
|---|---|
| baseline database | postgres://postgres:***@127.0.0.1:5433/rustdb_synth |
| command | C:\jason\dev\rust-db\target\release\rustdb-bench.exe grade --database-url postgres://postgres:rustdb@127.0.0.1:5433/rustdb_synth --cache J:/rust-db-embeddings/corpus.cache --model-dir J:/rust-db-embeddings/models/nomic-embed-text-v1.5 --device cuda:0 --per-source 100 --runs-dir J:/rust-db-embeddings/runs --out C:/jason/dev/rust-db/rust-db-scorecard.md |
| commit | 0c14f71d979e38345138479caff3988513d372fb (working tree dirty) |
| corpus cache | J:/rust-db-embeddings/corpus.cache |
| device | Cuda(0) |
| embedding model | J:/rust-db-embeddings/models/nomic-embed-text-v1.5/model.onnx |
| host | windows x86_64, 24 logical processors |
| per-query records | 8139 lines in J:/rust-db-embeddings/runs\1788237655-0c14f71d\per-query.jsonl |
...
| run manifest | J:/rust-db-embeddings/runs\1788237655-0c14f71d\manifest.json |
