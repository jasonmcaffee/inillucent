...
| **Smaller than SQLite** | **No - 1.43x its peak resident set, against a 0.95x bar the contract now carries.** It was 2.02x before task-1869, which bounded the redo buffer, stopped the index build holding three copies of the tree, collected a version log nothing was collecting, and capped the allocator's free list in bytes as well as blocks. What is left is not a buffer: this engine's *transient* is now smaller than SQLite's, and the whole gap is the `.rdb` being **1.41x** the `.db` - a PAX leaf spends a fixed 8-byte slot per column where SQLite spends a varint - plus a **4.25 MiB** larger process floor. The cache costs what the file costs | [Where the memory goes](feature-comparison.md#where-the-memory-goes) |
