# rust-db Technical Design Document

## 1. What this is and why it exists

rust-db is an embedded search engine for a corpus of workplace documents: wiki pages, chat messages, issue threads, source files, design files and boards. The usual way to build retrieval over such a corpus needs two separate programs running alongside the application. rust-db replaces both with one library that runs inside the calling process:

| Replaced | Replaced by |
|---|---|
| PostgreSQL 18.6 with the pgvector 0.8.6 extension | an in process vector index and inverted index owned by rust-db |
...
This is the specification rust-db has to meet. It is the behaviour of a retrieval stack built on PostgreSQL with pgvector and `llama.cpp` serving the embedding model, and `rustdb-bench` issues that stack's SQL directly so the baseline is real behaviour rather than an approximation of it.
...
2. **Grade the index, not the embeddings.** Both engines are loaded with byte identical vectors and the same chunk text. A score difference then measures indexing and ranking, which is what rust-db is responsible for.
3. **Exhaustive search is the ground truth.** Approximate accuracy is meaningless without an exact reference, so rust-db ships an exhaustive scan and the harness uses it to define correct answers.
...
rust-db/
  rust-db-tdd.md
  rust-db-scorecard.md              generated by the harness
  crates/
    rustdb-core/                    the engine, no I/O beyond the index files
      src/lib.rs
    ...
    rustdb-bench/                   the grading harness
      src/main.rs
...
`rustdb-core` has no knowledge of PostgreSQL. `rustdb-bench` is the only crate that links a database client, because its job is to read the baseline corpus and to query the baseline engine.
...
The difference is what happens when a predicate is present. Filtering after the search is what produces the zero row results in section 1.2. rust-db instead does what ACORN describes: traverse the graph as though it contained only the nodes that satisfy the predicate, without materializing that subgraph.
...
When the predicate is very selective, graph traversal stops being the right tool, so rust-db scans the passing set instead. An exhaustive scan of 17,000 normalized 768 dimensional vectors takes a few milliseconds and is exact, so on a selective predicate it is both faster and more accurate than a graph walk.
...
The pass condition is parity across all scenarios. Concretely: rust-db must be at least equal to the better of the two pgvector configurations on every accuracy family, with no correctness gate failing. Where rust-db is worse, the score card says so plainly, because a score card that cannot report a loss is not measuring anything.
...
**Document identity ground truth has a bias.** A title as a query favours engines that weight titles, and rust-db indexes chunk content just as the baseline does, so neither engine sees the title as text. The bias that remains is that titles share vocabulary with their own content, which flatters lexical retrieval slightly on both sides equally. It is reported as a caveat rather than hidden.
...
1. `rustdb-core`: storage, distance, exhaustive search, tokenization, BM25, Reciprocal Rank Fusion. Exhaustive search first, because it is the reference everything else is graded against.
2. `rustdb-bench`: load the corpus and the vectors once into a local cache file, so repeated runs do not repay the cost of reading them.
3. HNSW, then filtered traversal, graded against exhaustive search at each step.
4. Quantization and the Matryoshka ladder.
5. The full suite against both pgvector configurations and rust-db, then iterate on whatever loses.
6. The score card.
