...
//!
//! Invariant: the expected values are SQLite's, recorded by `rustdb-slt` from
...
use rustdb_compat::slt::{self, Record, TestFile};
...
fn corpus_copy(tag: &str) -> PathBuf {
    ...
    let directory = std::env::temp_dir().join(format!("rustdb-slt-{}-{tag}", std::process::id()));
...
fn render_row(row: &[OwnedDatum], types: &str) -> Vec<String> {
    ...
    row.iter()
        ...
        .map(|(index, value)| {
            ...
            let as_value = rustdb_exec::scalar::to_value(borrowed);
...
/// catalog tree.
///
/// The TDD asks Phase 2 for "`rustdb-catalog` over the catalog tree (read side)
