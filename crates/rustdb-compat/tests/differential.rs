//! rust-db and pinned SQLite 3.53.4, asked the same questions.
//!
...
//! statement would ask SQLite a different question from the one rust-db was
...
use rustdb_compat::corpus;
use rustdb_compat::fixtures::{malformed_fixtures, valid_fixtures};
...
use rustdb_value::cast;
use rustdb_value::collation::{self, Collation};
use rustdb_value::compare::{self, SqlOrdering};
use rustdb_value::record;
...
/// Turns a tagged value back into a rust-db value.
...
#[test]
fn sqlite_and_rustdb_agree_on_every_row_of_every_fixture() {
    ...
    for fixture in valid_fixtures() {
        ...
        for object in &scanned.objects {
            ...
            assert_eq!(
                answer.rows.len(),
                rows.len(),
                "{} / {}: SQLite returned {} rows and rust-db {}",
                fixture.name,
                object.name,
                answer.rows.len(),
                rows.len()
    ...
    for value in matrix_values() {
        for (name, affinity_target) in targets {
            ...
            let got = cast::cast_value(untagged(&value), affinity_target, TextEncoding::Utf8)
                .expect("rust-db casts");
            let got = tagged(&got);
            assert!(
                want.identical(&got),
                "CAST({value:?} AS {name}): SQLite says {want:?}, rust-db says {got:?}"
            );
            compared = compared.saturating_add(1);
        }
    }
    ...
    for value in matrix_values() {
        ...
        for (index, declaration) in declared.iter().enumerate() {
            ...
            let applied = affinity::apply_affinity(untagged(&value), target, TextEncoding::Utf8)
                .expect("rust-db applies the affinity");
...
fn comparison_matches_sqlite() {
    ...
    for left in &values {
        for right in &values {
            ...
            assert_eq!(
                expected, got,
                "{left:?} against {right:?}: SQLite says {expected:?}, rust-db says {got:?}"
            );
            compared = compared.saturating_add(1);
        }
    }
        ...
        for left in &texts {
            for right in &texts {
                ...
                assert_eq!(
                    expected,
                    got,
                    "{name}: {:?} against {:?}: SQLite says {expected:?}, rust-db says {got:?}",
...
fn every_value_survives_a_round_trip_through_a_file() {
    ...
    for encoding_name in ["UTF-8", "UTF-16le", "UTF-16be"] {
        ...
        let vfs = rustdb_vfs::OsVfs::new();
        let mut pager = rustdb_storage::pager::Pager::open_read_only(
            &vfs,
            &rustdb_vfs::DbPath::new(&path),
            rustdb_storage::pager::PagerOptions::default(),
        )
        .expect("rust-db opens the round-trip database");
        pager.begin_read().expect("a read starts");
        let scanned = corpus::scan_database(&mut pager).expect("rust-db scans it");
        ...
        // And the bytes rust-db would write for those values are the bytes
        // SQLite did write.
...
/// Every malformed fixture SQLite refuses, rust-db must also refuse.
///
...
/// wrong, and rust-db's job is to refuse the file, not to reproduce the exact
/// sentence.
#[test]
fn sqlite_and_rustdb_both_refuse_the_malformed_corpus() {
    let Some(mut oracle) = start_oracle() else {
        announce_skip();
        return;
    };
    for fixture in malformed_fixtures() {
        ...
        let vfs = rustdb_vfs::OsVfs::new();
        let path = rustdb_vfs::DbPath::new(corpus::fixture_path(fixture.name));
        let rustdb_refuses = match rustdb_storage::pager::Pager::open_read_only(
            &vfs,
            &path,
            rustdb_storage::pager::PagerOptions::default(),
        ) {
            ...
            Ok(mut pager) => {
                ...
                let check_failed = rustdb_storage::check::check_database(
                    &mut pager,
                    rustdb_storage::check::CheckLevel::Integrity,
                )
...
fn every_limit_matches_sqlites_default() {
    ...
    let ours = rustdb_base::limits::Limits::default();
    let mut compared = 0usize;
    for row in &answer.rows {
        ...
        let Some(limit) = rustdb_base::limits::LIMIT_ROWS
            .iter()
            .find(|row| row.c_name == name)
        else {
            // A limit rust-db does not model yet is a gap the manifest owns,
        ...
        assert_eq!(
            ...
            *value,
            "{name}: rust-db defaults to {} and SQLite to {value}",
            ours.get(limit.limit)
        );
        compared = compared.saturating_add(1);
    }
    ...
    assert_eq!(
        over.code,
        rustdb_base::PrimaryCode::TooBig.value(),
