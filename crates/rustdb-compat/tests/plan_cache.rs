...
fn scratch(name: &str) -> std::path::PathBuf {
    let root = rustdb_compat::workspace_root()
...
fn rows(connection: &rustdb::Connection, sql: &str) -> Vec<String> {
...
fn registering_a_function_drops_the_cached_programs() {
    ...
    connection
        .create_scalar_function(
            "twice",
            1,
            rustdb::extensions::FunctionFlags::default(),
...
fn a_lever_change_is_cached_separately() {
    ...
    connection.disable_optimizations(rustdb::Levers::COVERING_INDEX);
...
fn the_cache_changes_no_answer() {
    ...
    for disabled in [0u32, rustdb::Levers::PLAN_CACHE] {
        ...
        if disabled == rustdb::Levers::PLAN_CACHE {
            assert_eq!(
                connection.cached_plan_count(),
                0,
