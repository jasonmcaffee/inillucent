...
use rustdb_compat::differential::{compare, compare_queries, start_rustdb, Step};
...
fn column(name: &str, sql: &str) -> Vec<String> {
    ...
    while statement.step().expect("it steps") {
        ...
        rows.push(match value {
            ...
            rustdb_value::Value::Null => "NULL".to_string(),
            other => format!("{other:?}"),
        });
    }
    rows
...
fn a_virtual_table_joins() {
    ...
    let mut statement = rustdb_session::statement::Statement::prepare(
        &connection,
    ...
    while statement.step().expect("it steps") {
        ...
        rows.push(format!(
            "{:?}/{:?}",
            pair.first().and_then(rustdb_value::Value::as_integer),
            pair.get(1).and_then(rustdb_value::Value::as_integer)
        ));
    }
...
fn hidden_columns_stay_hidden() {
    let connection = start_rustdb(AREA, "hidden");
    let statement = rustdb_session::statement::Statement::prepare(
        &connection,
