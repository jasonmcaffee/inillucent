...
use rustdb_sim::media::MediaModel;
use rustdb_sim::schedule::{explore_two_actors, ActorId, Decisions, Scheduler};
use rustdb_sim::sim_vfs::{set_current_actor, SimConfig, SimVfs};
...
fn scratch(name: &str) -> std::path::PathBuf {
    let directory = workspace_root().join("_agent_output/task-1788/concurrency");
...
fn connect(path: &std::path::Path) -> rustdb::Connection {
    let database = Database::open(path).expect("the database opens");
    database.connect().expect("the connection opens")
}

/// Returns the single integer a query reports.
fn integer(connection: &rustdb::Connection, sql: &str) -> i64 {
...
fn fill(connection: &rustdb::Connection, rows: i64) {
    connection
...
fn run(connection: &Connection, sql: &str) -> Result<(), rustdb_base::DbError> {
...
fn scheduled_run(decisions: Decisions) -> (Outcome, Vec<usize>) {
    ...
    let (mut statement, _) =
        rustdb_session::statement::Statement::prepare(&reader, b"SELECT a FROM t ORDER BY a")
    ...
    let integrity = reader
        .with_database(rustdb_storage::MAIN_DATABASE, |pager| {
            pager.begin_read()?;
            let report = rustdb_storage::check::check_database(
                pager,
...
fn a_progress_handler_stops_a_long_statement() {
    ...
    assert_eq!(
        ...
        Err(rustdb_base::PrimaryCode::Interrupt),
