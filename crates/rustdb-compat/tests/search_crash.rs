...
use rustdb_sim::sim_vfs::{CrashSnapshot, SimConfig, SimVfs};
...
fn try_connect(vfs: Arc<dyn Vfs>, mode: JournalMode) -> Result<Connection, rustdb_base::DbError> {
...
fn query(connection: &Connection, sql: &str) -> Result<Vec<String>, rustdb_base::DbError> {
    let (mut statement, _) =
        rustdb_session::statement::Statement::prepare(connection, sql.as_bytes())?;
...
fn try_state(connection: &Connection) -> Result<Vec<String>, rustdb_base::DbError> {
...
fn record(name: &str, report: &str) {
    let directory = rustdb_compat::workspace_root().join("_agent_output/task-1790/search-crash");
...
fn a_crash_during_compaction_leaves_the_index_it_started_from() {
    ...
    let schema = "CREATE VIRTUAL TABLE docs USING rustdb_search(title, body, compact = 0);
         INSERT INTO docs(rowid, title, body) VALUES (1, 'Offer', 'who qualifies for the discount');
    ...
    let probe = |connection: &Connection| -> Result<Vec<String>, rustdb_base::DbError> {
        let mut state = query(
            connection,
