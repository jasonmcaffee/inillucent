...
//! binary, and rust-db reads it without ever having been told what is in it.
//! The strongest assertions in this file need no oracle at all, because the
//! *file* is the oracle: a record rust-db decodes and re-encodes must produce
...
fn each_fixture_has_the_page_size_and_encoding_its_name_declares() {
    for fixture in valid_fixtures() {
        ...
        let declared = rustdb_compat::fixtures::page_size_from_name(fixture.name).unwrap();
        assert_eq!(
...
fn the_vacuum_fixtures_have_pointer_maps_that_the_check_accounts_for() {
    for (name, expected) in [
        ("autovacuum-p1024-utf8.db", rustdb_storage::VacuumMode::Auto),
        (
            "incrvacuum-p1024-utf8.db",
            rustdb_storage::VacuumMode::Incremental,
        ),
        ("basic-p1024-utf8.db", rustdb_storage::VacuumMode::None),
    ] {
        ...
        if expected == rustdb_storage::VacuumMode::None {
...
fn each_collation_index_is_sorted_under_its_own_collation() {
    ...
    for (name, collation) in [
        ...
        let key = KeyInfo {
            columns: vec![rustdb_value::KeyColumn {
                collation,
                descending: false,
            }],
        };
...
/// other way round: rust-db must read it as cleanly as SQLite does, because
...
fn a_traversal_returns_every_pin_and_lock() {
    for fixture in valid_fixtures() {
        let (_vfs, mut pager) = corpus::open_fixture(fixture.name).unwrap();
        assert_eq!(pager.lock_level(), rustdb_vfs::FileLock::Shared);
        let _ = corpus::scan_database(&mut pager).unwrap();
        let _ = check::check_database(&mut pager, CheckLevel::Integrity).unwrap();
        assert_eq!(
        ...
        pager.end_read().unwrap();
        assert_eq!(pager.lock_level(), rustdb_vfs::FileLock::None);
        assert_eq!(
...
fn the_corpus_matches_its_manifest() {
    let output = std::process::Command::new(env!("CARGO_BIN_EXE_rustdb-fixtures"))
        .arg("--verify")
        .output()
