...
//!
//! Invariant: rust-db's work is judged by SQLite and SQLite's by rust-db, never
...
/// Runs a script through rust-db.
fn rustdb(path: &Path, sql: &str) {
    let database = Database::open(path).expect("rust-db opens the file");
    let connection = database.connect().expect("rust-db connects");
    connection
        .execute_batch(sql)
        .expect("rust-db runs the script");
}

/// Returns the rows a rust-db query produces, rendered.
fn query(path: &Path, sql: &str) -> Vec<String> {
    let database = Database::open(path).expect("rust-db opens the file");
    let connection = database.connect().expect("rust-db connects");
    connection
        .query(sql)
        .expect("rust-db queries")
        .iter()
        .map(|row| {
            row.iter()
...
/// A database rust-db built from nothing is one SQLite reads and approves of.
#[test]
fn sqlite_reads_what_rustdb_wrote() {
    if pinned_shell().is_none() {
        announce_skip();
        return;
    }
    let path = scratch("rustdb-wrote");
    rustdb(
        &path,
    ...
    // The index rust-db built is one SQLite uses and agrees with.
    let by_index =
...
/// SQLite can keep writing to a database rust-db wrote, and the result is one
/// rust-db still reads.
#[test]
fn sqlite_writes_on_top_of_rustdb_and_rustdb_reads_it_back() {
    if pinned_shell().is_none() {
        announce_skip();
        return;
    }
    ...
    // The index SQLite maintained is one rust-db seeks through.
    let by_index = query(&path, "SELECT a FROM t WHERE b = 'three'");
    assert_eq!(by_index, vec!["3"]);

    // And rust-db can keep writing after SQLite has.
    rustdb(
        &path,
...
/// A database SQLite built is one rust-db writes into, and SQLite still
/// approves of the result.
#[test]
fn rustdb_writes_into_a_database_sqlite_built() {
    if pinned_shell().is_none() {
        announce_skip();
        return;
    }
    ...
    // The `sku` primary key is an automatic index SQLite created, and rust-db
...
/// A table rust-db created and dropped leaves a database SQLite still accepts.
#[test]
...
/// A transaction rust-db rolled back leaves nothing SQLite can see.
#[test]
