//! rust-db and the pinned SQLite 3.53.4, asked the same JSON questions.
//!
...
use rustdb_compat::differential::{compare_queries, Step};
...
fn documents_survive_a_round_trip_through_a_table() {
    ...
    let compared = rustdb_compat::differential::compare(AREA, "table", &steps);
    if compared == 0 {
        return;
    }
