...
//! rust-db's work is judged by SQLite's `PRAGMA integrity_check` and by what
//! SQLite reads back, and SQLite's work is judged by rust-db's own raw
//! traversal.
//!
...
//!
//! The tables rust-db writes into are ones it creates itself, and that is not
...
use rustdb_compat::{fixtures, workspace_root};
use rustdb_storage::check::{self, CheckOptions};
use rustdb_storage::cursor::BTreeCursor;
use rustdb_storage::header::VacuumMode;
use rustdb_storage::pager::{NewDatabase, Pager, PagerOptions};
use rustdb_storage::{mutate, schema};
use rustdb_value::record::{encode_record, RecordRef};
use rustdb_value::{BlobValue, TextEncoding, Value};
use rustdb_vfs::{DbPath, OsVfs};

/// The name of the table rust-db creates in a file SQLite wrote.
const ADDED_TABLE: &str = "rustdb_added";

/// The SQL that describes it, which is what goes in `sqlite_schema`.
const ADDED_SQL: &str = "CREATE TABLE rustdb_added(a)";

/// Returns the pinned SQLite shell, or `None` when it has not been downloaded.
fn pinned_shell() -> Option<PathBuf> {
    if let Ok(explicit) = std::env::var("RUSTDB_SQLITE_SHELL") {
...
fn scratch(name: &str) -> PathBuf {
    let directory = std::env::temp_dir().join("rustdb-interop").join(name);
...
/// Adds a table to a database and fills it, entirely through rust-db.
///
...
fn add_table_with_rustdb(path: &Path, rows: &[(i64, usize)]) -> Result<(), String> {
    let vfs = OsVfs::new();
    ...
    // Delete a third of them again so the file carries a freelist rust-db built.
...
/// Reads a table's rows back through rust-db as `(rowid, payload)`.
...
/// Runs rust-db's own integrity check over a whole database.
fn rustdb_integrity(path: &Path) -> Result<(), String> {
    let vfs = OsVfs::new();
...
/// SQLite writes a file, rust-db adds a table to it, and SQLite checks and
/// reads the result.
#[test]
fn rustdb_mutates_a_sqlite_file_and_sqlite_accepts_it() {
    let Some(shell) = pinned_shell() else { return };
    let corpus = fixtures::corpus_root(&workspace_root());
    let directory = scratch("rustdb-writes");
    let names = [
        "basic-p512-utf8.db",
        "basic-p1024-utf8.db",
        "basic-p4096-utf8.db",
    ...
    for name in names {
        ...
        if !source.is_file() {
            eprintln!("the fixture corpus is not built; run rustdb-fixtures");
            return;
        }
        let target = directory.join(name);
        std::fs::copy(&source, &target).expect("a copy of the fixture");

        // SQLite says the file is sound before rust-db touches it.
        ...
        add_table_with_rustdb(&target, &rows).unwrap_or_else(|reason| panic!("{name}: {reason}"));
        ...
        assert_eq!(
            after.trim(),
            "ok",
            "{name}: SQLite refused the file rust-db wrote"
        );
...
/// rust-db writes a whole database from nothing, and SQLite opens it, checks
/// it, reads it, writes to it, and checks it again - after which rust-db reads
/// what SQLite wrote.
#[test]
fn sqlite_mutates_a_rustdb_file_and_rustdb_accepts_it() {
    let Some(shell) = pinned_shell() else { return };
    ...
    for (page_size, vacuum) in [
        ...
        let db_path = DbPath::new(&path);
        {
            let mut pager = Pager::create(
                ...
                NewDatabase {
                    page_size: rustdb_base::page::PageSize::new(page_size).expect("a page size"),
                    reserved_bytes: 0,
        ...
        add_table_with_rustdb(&path, &rows).unwrap_or_else(|reason| panic!("{label}: {reason}"));
        ...
        assert_eq!(
            checked.trim(),
            "ok",
            "{label}: SQLite refused a database rust-db created from nothing"
        );

        // SQLite now writes to it: new rows, an index over the whole table, and
        // deletes that put pages back on the freelist rust-db built.
        ...
        // rust-db reads what SQLite wrote, and its own traversal agrees.
        rustdb_integrity(&path).unwrap_or_else(|reason| panic!("{label}: {reason}"));
        ...
        let object = schema::find_object(&mut pager, ADDED_TABLE)
            .unwrap_or_else(|error| panic!("{label}: {error:?}"))
            .unwrap_or_else(|| panic!("{label}: the table rust-db created is gone"));
        ...
        assert_eq!(
            found.len() as i64,
            expected,
            "{label}: rust-db and SQLite disagree about how many rows there are"
        );
...
/// depends on the other's *layout* - that rust-db can balance a page SQLite
/// split, and that SQLite can insert into a page rust-db laid out its own way.
#[test]
fn the_two_engines_take_turns_writing_the_same_file() {
    ...
    let db_path = DbPath::new(&path);
    {
        let mut pager = Pager::create(
            ...
            PagerOptions::default(),
            NewDatabase {
                page_size: rustdb_base::page::PageSize::new(1024).expect("a page size"),
                reserved_bytes: 0,
                text_encoding: TextEncoding::Utf8,
    ...
    add_table_with_rustdb(&path, &added_rows()).expect("rust-db writes first");

    for round in 0..3 {
        // A base that cannot collide with the rows already there, or with the
        // ones rust-db adds below: each engine writes into its own band.
        ...
        rustdb_integrity(&path).unwrap_or_else(|reason| panic!("round {round}: {reason}"));

        // rust-db's turn: more rows, and deletes that reuse SQLite's freelist.
        let mut pager =
        ...
        assert_eq!(
            checked.trim(),
            "ok",
            "round {round}: SQLite refused the file after rust-db's turn"
        );
    }
}
