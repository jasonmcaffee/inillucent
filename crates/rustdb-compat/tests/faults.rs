...
use rustdb_transaction::journal::JournalOptions;
...
fn rows(connection: &Connection) -> Vec<String> {
    let (mut statement, _) =
        rustdb_session::statement::Statement::prepare(connection, b"SELECT a, c FROM t ORDER BY a")
