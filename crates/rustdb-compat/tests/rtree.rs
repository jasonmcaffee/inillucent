...
use rustdb_compat::differential::{compare, sqlite_oracle, Step};
...
fn a_split_tree_answers_every_query() {
    let path = rustdb_compat::differential::scratch(AREA, "deep", "rustdb");
    build_with_rustdb(&path, 400);
    assert_eq!(
        query_with_rustdb(&path, "SELECT count(*) FROM spots"),
        ["400"]
    );
    assert_eq!(
        query_with_rustdb(
    ...
    assert_eq!(
        query_with_rustdb(&path, "SELECT id FROM spots WHERE id = 250"),
        ["250"]
    );
    ...
    // root.
    let nodes = query_with_rustdb(&path, "SELECT count(*) FROM spots_node");
    assert!(
        nodes
            .first()
    ...
    rustdb_session::statement::execute_batch(&connection, b"DELETE FROM spots WHERE id > 200")
        .expect("deletes");
    drop(connection);
    assert_eq!(
        query_with_rustdb(&path, "SELECT count(*) FROM spots"),
    ...
    assert_eq!(query_with_rustdb(&path, "PRAGMA integrity_check"), ["ok"]);
}

/// A tree rust-db built is one the pinned release opens, queries and writes.
#[test]
fn the_pinned_release_reads_what_rust_db_wrote() {
    let Some(shell) = shell() else {
        return;
    };
    let _ = shell;
    let path = rustdb_compat::differential::scratch(AREA, "cross-out", "rustdb");
    build_with_rustdb(&path, 200);
    ...
    // And it writes: a row it adds is one rust-db then finds.
    run_shell(
        &path,
        "INSERT INTO spots VALUES (9999, 500.0, 501.0, 500.0, 501.0);",
    );
    let found = query_with_rustdb(&path, "SELECT id FROM spots WHERE minX >= 499.0");
    assert_eq!(found, vec!["9999"], "rust-db reads the row SQLite added");
}

/// A tree the pinned release built is one rust-db opens, queries and writes.
#[test]
fn rust_db_reads_what_the_pinned_release_wrote() {
    let Some(shell) = shell() else {
        return;
    };
    let _ = shell;
    let path = rustdb_compat::differential::scratch(AREA, "cross-in", "sqlite");
    ...
    let counted = query_with_rustdb(&path, "SELECT count(*) FROM spots");
    assert_eq!(counted, vec!["200"]);
    let found = query_with_rustdb(
        &path,
        "SELECT id FROM spots WHERE minX >= 100.0 AND maxX <= 104.0 ORDER BY id",
    );
    assert_eq!(found, vec!["100", "101", "102", "103"]);

    // And rust-db writes: a row it adds is one the pinned release then finds.
    let connection = open_existing(&path);
    rustdb_session::statement::execute_batch(
        &connection,
        b"INSERT INTO spots VALUES (9999, 500.0, 501.0, 500.0, 501.0)",
    )
    .expect("rust-db inserts");
    drop(connection);
    let listed = run_shell(&path, "SELECT id FROM spots WHERE minX >= 499.0;");
    assert_eq!(listed.trim(), "9999");
}

/// Builds a tree with rust-db, from nothing.
fn build_with_rustdb(path: &PathBuf, rows: usize) {
    let database = rustdb_session::connection::SessionDatabase::open_with_options(
        path,
        rustdb_session::connection::OpenOptions::default(),
    )
    .expect("rust-db opens");
    let connection = database.connect().expect("rust-db connects");
    rustdb_session::statement::execute_batch(
        &connection,
        b"CREATE VIRTUAL TABLE spots USING rtree(id, minX, maxX, minY, maxY)",
    )
    .expect("the table is made");
    rustdb_session::statement::execute_batch(&connection, b"BEGIN").expect("begins");
    for index in 1..=rows {
        let sql = format!(
            "INSERT INTO spots VALUES ({index}, {index}.0, {}.0, {index}.0, {}.0)",
            index + 1,
        ...
        rustdb_session::statement::execute_batch(&connection, sql.as_bytes()).expect("inserts");
    }
    rustdb_session::statement::execute_batch(&connection, b"COMMIT").expect("commits");
}

/// Opens a database rust-db did not create.
fn open_existing(path: &PathBuf) -> rustdb_session::connection::Connection {
    let database = rustdb_session::connection::SessionDatabase::open_with_options(
        path,
        rustdb_session::connection::OpenOptions::default(),
    )
...
/// Runs one query with rust-db and returns its first column as text.
fn query_with_rustdb(path: &PathBuf, sql: &str) -> Vec<String> {
    let connection = open_existing(path);
    let mut statement = rustdb_session::statement::Statement::prepare(&connection, sql.as_bytes())
        .expect("it prepares")
        .0;
    let mut rows = Vec::new();
    while statement.step().expect("it steps") {
        rows.push(match statement.value(0) {
            rustdb_value::Value::Integer(number) => number.to_string(),
            rustdb_value::Value::Real(number) => number.to_string(),
            rustdb_value::Value::Text(text) => {
