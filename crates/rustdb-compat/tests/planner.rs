...
//! plan.
//!
//! The `sqlite_stat1` rust-db writes is read by the pinned 3.53.4 binary and
...
fn plan(connection: &rustdb::Connection, sql: &str) -> Vec<String> {
...
fn build(connection: &rustdb::Connection) {
    run_all(
        connection,
        &[
...
/// Statistics the pinned binary gathered are read by rust-db.
#[test]
fn statistics_sqlite_wrote_are_read_back() {
    ...
    assert!(
        ...
        "rust-db did not use SQLite's statistics: {planned:?}"
    );
