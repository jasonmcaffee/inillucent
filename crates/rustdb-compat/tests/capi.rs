...
fn compile(name: &str, headers: &Path, library: &Path) -> Option<PathBuf> {
    ...
    let source = workspace_root().join("crates/rustdb-compat/tests/c/capi_probe.c");
...
fn the_c_probe_agrees_with_the_pinned_engine() {
    ...
    let Some(against_rustdb) = compile("probe_rustdb", &headers, &library) else {
        eprintln!("no usable C compiler; skipping");
        return;
    };
    let (Some(expected), Some(found)) = (run(&against_sqlite), run(&against_rustdb)) else {
