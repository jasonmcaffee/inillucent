...
fn scratch(name: &str) -> PathBuf {
    let directory = workspace_root().join("_agent_output/task-1788/temp");
...
/// Runs a script through rust-db on one connection, statement by statement.
...
fn report(connection: &rustdb::Connection, script: &str) -> String {
...
fn grade(name: &str, script: &str) {
    ...
    let candidate = run(&scratch(&format!("{name}-rustdb")), script);
    assert_eq!(
        normalise(&reference),
        normalise(&candidate),
        "\n--- script\n{script}\n--- sqlite\n{reference}\n--- rust-db\n{candidate}"
...
fn each_connection_has_its_own_temporary_database() {
    ...
    let count = |connection: &rustdb::Connection| {
        connection
            .query("SELECT count(*) FROM scratch")
            .expect("the query runs")
            .first()
