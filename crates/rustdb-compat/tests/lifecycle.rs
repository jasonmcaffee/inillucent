...
use rustdb::{Database, PrimaryCode, Value};
use rustdb_base::limits::Limits;
use rustdb_compat::oracle::{Driver, Op, TaggedValue};
use rustdb_compat::workspace_root;
use rustdb_vm::machine::Machine;
use rustdb_vm::program::{Instruction, Opcode, Operand, Program, ProgramDependencies};
use rustdb_vm::{verify, verify_operands};
...
fn the_machine_runs_a_verified_program() {
    ...
    assert_eq!(machine.state(), rustdb_vm::MachineState::Prepared);
    machine.reset();
