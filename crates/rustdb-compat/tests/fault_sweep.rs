...
//! contract forbids a production crate from naming `rustdb-sim` at all - "even
//! as a dev-dependency, which puts harness code in the engine's test surface".
//! That rule is right and this file is where it points: `rustdb-compat` already
...
use rustdb_transaction::recovery::{create_database, open_database, DatabaseOptions};
