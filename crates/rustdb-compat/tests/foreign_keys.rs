...
/// separated and the refusals land after the rows. [`normalise`] puts rust-db's
...
/// comparable at all.
fn run(connection: &rustdb::Connection, script: &str) -> String {
...
fn grade(name: &str, script: &str) {
    ...
    let candidate = run(&connect(&scratch(&format!("{name}-rustdb"))), script);
