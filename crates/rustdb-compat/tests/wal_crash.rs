...
fn try_connect(vfs: Arc<dyn Vfs>) -> Result<Connection, rustdb_base::DbError> {
...
fn try_contents(connection: &Connection) -> Result<Vec<String>, rustdb_base::DbError> {
    let (mut statement, _) = rustdb_session::statement::Statement::prepare(
        connection,
...
fn record(name: &str, body: &str) {
    let directory = rustdb_compat::workspace_root().join("tests/crash");
