...
/// Opens a rust-db connection on a path.
fn connect(path: &Path) -> rustdb::Connection {
...
/// SQLite reads a WAL database rust-db wrote, with the transactions still only
...
#[test]
fn sqlite_reads_a_log_rustdb_wrote() {
...
/// rust-db reads and extends a WAL database SQLite wrote and closed.
///
...
/// marked WAL, rust-db opens it in WAL mode without being told to, and what it
/// writes afterwards is readable by SQLite again.
#[test]
fn rustdb_extends_a_wal_database_sqlite_wrote() {
    let Some(_) = pinned_shell() else {
        eprintln!("the pinned shell is not present; skipping");
        return;
    };
...

    {
        ...
        connection
            ...
            .expect("rust-db writes to SQLite's database");
    }
...
/// rust-db recovers a log a SQLite writer died holding.
///
...
#[test]
fn rustdb_recovers_a_log_a_killed_sqlite_left() {
    ...
    connection
        .execute_batch("INSERT INTO t VALUES (3, 'three')")
        .expect("rust-db writes to the log SQLite left");
    drop(connection);
