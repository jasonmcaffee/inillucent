...
//!   `PRAGMA rustdb.force_plan` alternative must produce the same digest.
//!
...
use rustdb_exec::physical::{ForcePlan, Params, SourceLayout, TreeCatalog};
...
struct Fixture {
    ...
    /// were built to match.
    catalog: rustdb_sql::catalog_view::StaticCatalog,
}
...
impl Fixture {
    ...
    fn plan(&self, sql: &str) -> rustdb_base::DbResult<rustdb_sql::plan::PhysicalPlan> {
        let limits = rustdb_base::limits::Limits::default();
        let parsed = rustdb_sql::parser::parse_next_statement(sql.as_bytes(), 0, &limits)
            .map_err(|error| rustdb_base::error::misuse(format!("{sql}: {error:?}")))?;
        let authorizer = rustdb_sql::bind::AllowAll;
        let mut binder = rustdb_sql::bind::Binder::new(&self.catalog, &parsed.ast, &authorizer)
            .with_source(sql.as_bytes());
        let bound = binder
            .bind_statement(&parsed.statement)
            .map_err(|error| rustdb_base::error::misuse(format!("{sql}: {error:?}")))?;
        match bound {
            rustdb_sql::bind::BoundStatement::Select(select) => Ok(
                rustdb_sql::plan::plan_select_with(*select, rustdb_sql::plan::Levers::default()),
            ),
            _ => Err(rustdb_base::error::misuse(format!(
...
fn fixture(rows: i64, page_size: usize, frames: usize) -> Fixture {
    ...
    let catalog = rustdb_sql::catalog_view::StaticCatalog::empty().with_table(info);
...
fn the_key_encoding_orders_like_the_comparison() {
    ...
    for left in &tuples {
        for right in tuples.iter().take(64) {
            ...
            let by_value = rustdb_tree::leaf::compare_rows(&a, &b, 1);
...
/// each applicable `PRAGMA rustdb.force_plan` alternative and must produce the
...
fn every_forced_plan_gives_the_same_answer() {
    ...
    for sql in queries {
        ...
        for (lever, forced) in ForcePlan::alternatives() {
            let Ok(prepared) = rustdb_exec::physical::prepare(&plan, &fixture, forced) else {
                continue;
            };
            let rows = Rc::new(RefCell::new(Vec::new()));
            let sink = Box::new(rustdb_exec::ops::CollectInto::new(Rc::clone(&rows)));
            let built = rustdb_exec::physical::build_prepared(
                &plan,
                &fixture,
                &prepared,
                &Params::new(),
...
fn the_physical_pass_refuses_what_it_cannot_run() {
    ...
    for (what, sql) in refused {
        ...
        let outcome = rustdb_exec::physical::run(&plan, &fixture, &Params::new());
        assert!(
            outcome.is_err(),
...
fn the_physical_pass_answers_what_it_implements() {
    ...
    for (sql, wanted) in cases {
        ...
        let (rows, shape) = rustdb_exec::physical::run(&plan, &fixture, &Params::new())
            .unwrap_or_else(|error| panic!("{sql}: {:?}", error.detail()));
...
fn parameters_reach_every_place_a_plan_uses_one() {
    ...
    for key in [0i64, 1, 250, 499, 500, -1] {
        ...
        let (rows, _) = rustdb_exec::physical::run(&plan, &fixture, &params).expect("it runs");
...
/// `PRAGMA rustdb.force_plan` parses its operator list and refuses a name it
/// does not know.
#[test]
