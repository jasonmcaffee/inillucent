//! Reading the fixture corpus, and scanning a database with rust-db alone.
//!
//! Invariant: everything here goes through the public `rustdb-storage` API, in
...
//! ships.
//!
//! The one piece of knowledge this module has that `rustdb-storage` does not
...
use rustdb_storage::schema::{self, SchemaKind, SchemaObject};
...
/// Everything one fixture holds, as rust-db reads it.
#[derive(Clone, Debug)]
...
pub fn open_fixture(name: &str) -> DbResult<(OsVfs, Pager)> {
    ...
    for attempt in 0..200u32 {
        match pager.begin_read() {
            ...
            Err(error) if error.code() == rustdb_base::PrimaryCode::Busy => {
...
pub fn scan_tree(
    ...
    let mut cursor = if kind_is_table {
        ...
        BTreeCursor::index(root, rustdb_value::KeyInfo::default())
    };
...
pub fn root_is_table(pager: &mut Pager, root: PageId) -> DbResult<bool> {
    ...
    let layout = rustdb_storage::PageLayout::parse(pin.bytes(), root, usable)?;
