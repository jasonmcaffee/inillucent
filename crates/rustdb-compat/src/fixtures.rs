...
//! binary and by nothing else. A fixture rust-db produced would prove that
//! rust-db agrees with itself, which is the one thing a parity corpus must not
...
pub struct MalformedFixture {
    ...
    /// SQLite reads happily. rust-db has to agree, and a corpus entry that
