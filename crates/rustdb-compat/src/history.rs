...
pub struct Entry {
    ...
    pub family: String,
    /// The point estimate of the speed ratio, SQLite over rust-db.
    pub ratio: f64,
...
pub fn dashboard(history: &History, platform: &str) -> String {
    ...
    out.push_str("# rust-db performance dashboard\n\n");
    out.push_str(&format!(
        "Platform `{platform}`. Every number is the paired speed ratio, SQLite over rust-db, so \
