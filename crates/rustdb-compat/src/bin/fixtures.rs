...
//!
//! Usage:
//!   `cargo run -p rustdb-compat --bin rustdb-fixtures`             regenerate
//!   `cargo run -p rustdb-compat --bin rustdb-fixtures -- --verify` check only

use std::path::{Path, PathBuf};
use std::process::{Command, ExitCode};

use rustdb_compat::fixtures::{
...
fn run(root: &Path, verify: bool) -> Result<String, String> {
    ...
    manifest.push_str(
        "# The fixture corpus.\n\
         ...
         # crates/rustdb-compat/src/fixtures.rs. Regenerate with:\n\
         #   cargo run -p rustdb-compat --bin rustdb-fixtures\n\
