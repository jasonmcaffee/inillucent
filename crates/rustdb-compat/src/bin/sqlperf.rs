...
//!
//! Usage: `cargo run --release -p rustdb-compat --bin rustdb-sqlperf`
...
///
/// rust-db cannot write yet, so the data these baselines read has to come from
...
fn run(root: &Path) -> Result<usize, String> {
    ...
    measurements.push(measure("lex", "representative", 20_000, || {
        let mut lexer = rustdb_sql::Lexer::new(REPRESENTATIVE.as_bytes());
        let mut tokens = 0u64;
        while let Ok(token) = lexer.next_token() {
            if token.kind == rustdb_sql::TokenKind::EndOfInput {
                break;
            }
