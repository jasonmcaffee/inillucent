...
//!
//! - **compiled** - `rustdb_exec::compile`, which picks a specialised node per
...
//!
//! Usage: rustdb-exprperf [rounds]

use std::time::Instant;

use rustdb_base::DbResult;
use rustdb_exec::batch::{Batch, Vector};
use rustdb_exec::expr::{compile, truth, ArithOp, CompareOp, Expr, StaticType};
...
fn run(rounds: u32) -> DbResult<()> {
    ...
    for (name, expr) in &predicates {
        ...
        for row in 0..ROWS {
            ...
            {
                return Err(rustdb_base::error::misuse(format!(
...
fn time(rounds: u32, mut body: impl FnMut() -> DbResult<u32>) -> DbResult<f64> {
    ...
    for _ in 0..rounds {
        ...
        if out != warm {
            return Err(rustdb_base::error::misuse("an arm was not deterministic"));
        }
    }
        ...
        Expr::Arith(op, left, right) => {
            ...
            if a.is_null() || b.is_null() {
                ...
                match (a.as_int(), b.as_int()) {
                    ...
                    _ => {
                        let (x, y) = (
                            rustdb_exec::expr::numeric(&a),
                            rustdb_exec::expr::numeric(&b),
                        );
                        Datum::Real(match op {
                            ArithOp::Add => x + y,
