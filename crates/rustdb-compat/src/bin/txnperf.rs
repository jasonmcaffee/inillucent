...
//!
//! Invariant: every number here is measured, and every number here is rust-db
...
//!
//! Usage: `cargo run --release -p rustdb-compat --bin rustdb-txnperf`
...
use rustdb_storage::pager::PagerCounters;
use rustdb_storage::JournalStats;
...
fn main() -> ExitCode {
    ...
    let scratch = flag(&arguments, "--scratch")
        .unwrap_or_else(|| workspace_root().join("_agent_output/task-1786/txnperf"));
...
fn crashed_image(options: JournalOptions, cut: u64) -> Result<Option<(Vec<u8>, Vec<u8>)>, String> {
    ...
    let replayable = rustdb_transaction::journal::decode_journal(&hot)
        .map_err(text)?
        .is_some();
