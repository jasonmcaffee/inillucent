...
//! workload through their own front door - rust-db through the C ABI it now
...
//!
//! Usage: `cargo run --release -p rustdb-compat --bin rustdb-featureperf -- [--out <dir>]`
...
use rustdb_session::connection::{OpenOptions, SessionDatabase};
...
struct Measurement {
    ...
    /// How many times.
    iterations: u32,
    /// What rust-db took, in nanoseconds per iteration.
    rustdb_nanos: f64,
...
fn open() -> Result<rustdb_session::Connection, String> {
...
fn exec(connection: &rustdb_session::Connection, sql: &str) -> Result<(), String> {
    rustdb_session::statement::execute_batch(connection, sql.as_bytes())
        .map_err(|error| format!("{sql}: {}", error.message()))
}

/// Steps a statement to completion, returning how many rows it produced.
fn drain(connection: &rustdb_session::Connection, sql: &str) -> Result<u32, String> {
...
fn api_overhead() -> Result<Measurement, String> {
    ...
    let rustdb = time(iterations, || drain(&connection, "SELECT 1").map(|_| ()))?;
    Ok(Measurement {
...
fn virtual_scan() -> Result<Measurement, String> {
    ...
    let rustdb = time(iterations, || drain(&connection, sql).map(|_| ()))?;
    Ok(Measurement {
...
fn cli_family() -> Result<Vec<Measurement>, String> {
    let Some(ours) = shell_binary("rustdb-shell") else {
        return Ok(Vec::new());
    };
    ...
    let ours_import = run_shell(&ours, &area.join("perf-rustdb.db"), &script);
    ...
    if let Some(elapsed) = ours_import {
        measurements.push(Measurement {
            ...
            iterations: rows,
            rustdb_nanos: per_iteration(elapsed, rows),
            sqlite_nanos: sqlite_import.map(|taken| per_iteration(taken, rows)),
        });
    }
    let dump = ".dump\n";
    let ours_dump = run_shell(&ours, &area.join("perf-rustdb.db"), dump);
...
fn table(measurements: &[Measurement]) -> String {
    ...
    out.push_str(
        "Both engines run the same workload through their own front door. The\n\
         ratio is rust-db over the reference, so below one is faster. A missing\n\
         reference number means the workload has no comparable single-process\n\
         form in the shell, not that it was skipped.\n\n",
    );
    out.push_str("| family | operation | iterations | rust-db ns | SQLite ns | ratio |\n");
    out.push_str("|---|---|---:|---:|---:|---:|\n");
