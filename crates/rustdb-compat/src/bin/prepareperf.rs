...
//!
//! Usage: rustdb-prepareperf <sqlite fixture> [rounds]

use std::path::{Path, PathBuf};
use std::process::{Command, ExitCode};
use std::time::Instant;

use rustdb::{Database, Levers, Value};
use rustdb_compat::perf::{bootstrap, median, Digest, Sample};
...
fn main() -> ExitCode {
    ...
    let Some(fixture) = arguments.first() else {
        eprintln!("usage: rustdb-prepareperf <sqlite fixture> [rounds]");
...
/// Runs both rust-db arms and the SQLite arm, interleaved.
///
...
fn run(fixture: &Path, rounds: u32) -> Result<(), String> {
    ...
    for round in 0..rounds {
        ...
        for step in 0..3u32 {
            match (order + step) % 3 {
                0 => {
                    let samples = time_rustdb(fixture, 0)?;
                    record(&samples, &mut cached, &mut digests)?;
                }
                1 => {
                    let samples = time_rustdb(fixture, Levers::PLAN_CACHE)?;
...
/// Times the rust-db arm at one lever mask.
///
/// @param fixture - the database to open
/// @param disabled - the levers to switch off
fn time_rustdb(fixture: &Path, disabled: u32) -> Result<Vec<Sample>, String> {
    let database = Database::open_with_busy_timeout(fixture, std::time::Duration::from_secs(5))
        .map_err(|error| format!("open: {}", error.message()))?;
    let connection = database
        .connect()
