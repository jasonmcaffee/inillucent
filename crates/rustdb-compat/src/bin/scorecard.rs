...
// The plans.
//
// They live in `rustdb_compat::perf` rather than here, because the Phase 2
...
fn render_markdown(
    ...
    out.push_str(
        "That is *not* automatic for the vectorised engine, and task-1816's Phase 1 numbers were \
         ...
         at the plan's 2 MB cache. Phase 2's gate (`rustdb-readgate`) closes it by deriving \
         SQLite's `cache_size` from the byte size of rust-db's own buffer pool, so `--frames` \
