...
//!
//! Usage: `cargo run --release -p rustdb-compat --bin rustdb-perf -- [--out <dir>]`
...
fn run(out: &Path) -> Result<String, String> {
    ...
    workspace.push(format!("rustdb-perf-{}", std::process::id()));
...
fn measure_allocations(vfs: &dyn Vfs, root: &DbPath) -> Result<(u64, u64, u64), String> {
    ...
    let size = rustdb_base::page::PageSize::DEFAULT;
    let pages = 1024u64;
