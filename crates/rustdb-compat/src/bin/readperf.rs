...
//!   actually lazy.
//!
//! Usage: `cargo run --release -p rustdb-compat --bin rustdb-readperf`
...
///
/// The databases are written by SQLite rather than by rust-db on purpose: a
/// baseline measured against a file rust-db laid out would be measuring
/// rust-db's own layout choices, and there are not any yet.
