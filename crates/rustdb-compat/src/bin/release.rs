...
use rustdb_base::hash::Sha256;
use rustdb_compat::history::History;
use rustdb_compat::perf::Contract;
...
const COMMANDS: [(&str, &str); 9] = [
    ...
    (
        "the test evidence",
        "cargo run --release -p rustdb-compat --bin rustdb-evidence",
    ),
    (
        "the compatibility report",
        "cargo run --release -p rustdb-compat --bin rustdb-manifest",
    ),
    (
        "the performance scorecard",
        "cargo run --release -p rustdb-compat --bin rustdb-scorecard -- --scale all --rounds 30 \
         --label <name>",
    ),
    (
        "each optimization's A/B arm",
        "cargo run --release -p rustdb-compat --bin rustdb-scorecard -- --scale small --rounds 30 \
         --label <name> --disable covering-index   # then --disable indexed-write",
    ),
    (
        "the storage and write profiles",
        "cargo run --release -p rustdb-compat --bin rustdb-storageprofile && cargo run --release \
         -p rustdb-compat --bin rustdb-writeprofile",
    ),
    (
        "a legacy index migration",
        "cargo run --release -p rustdb-migrate -- <index-dir> <destination.db> --sqlite \
         .sqlite-ref/3.53.4/shell/sqlite3",
    ),
    (
        "this release candidate",
        "cargo run --release -p rustdb-compat --bin rustdb-release",
    ),
];
...
fn run(out: &Path) -> Result<bool, String> {
    ...
    let library = if cfg!(windows) {
        "target/release/rustdb_capi.dll"
    } else {
        "target/release/librustdb_capi.so"
    };
    let wanted: Vec<(&str, String, &str)> = vec![
        (
            "cli",
            format!("target/release/rustdb-shell{suffix}"),
            "the SQLite-like shell",
        ),
        (
            "migrate",
            format!("target/release/rustdb-migrate{suffix}"),
        ...
        (
            "baseline",
            "compat/baseline/rustdb-core-baseline.json".to_string(),
            "the retrieval engine's frozen baseline",
        ),
        (
            "amendments",
            "compat/baseline/rustdb-core-amendments.toml".to_string(),
...
fn render(
    ...
    out.push_str("# rust-db release candidate\n\n");
    ...
    out.push_str(
        "A clean machine with a Rust toolchain and a C compiler reproduces every artifact above \
         ...
         built from source into `.sqlite-ref/`, which no rust-db crate links against.\n\n",
    );
    ...
    out.push_str(
        "The default writer produces the SQLite file format and nothing else, so an upgrade is a \
         ...
         `rustdb_search` table is read by any build - the index lives in ordinary tables - but it \
         is *queried* only by a build that has the module. An older build opens the file, reads \
         every relational table, and reports `no such module: rustdb_search` for the search table \
         alone.\n\n\
         Legacy retrieval indexes migrate with `rustdb-migrate`, which never writes to the \
...
fn limitations(history: &History) -> String {
    ...
    out.push_str(
        "- **A `rustdb_search` table has one row per document**, so the per-document cap in the \
