...
//!
//! Usage: `cargo run --release -p rustdb-compat --bin rustdb-storageprofile`
...
use rustdb_value::record::{encode_record, KeyColumn, KeyInfo, RecordRef};
...
fn build() -> Result<(Pager, rustdb_base::ids::PageId, rustdb_base::ids::PageId), String> {
...
fn run() -> Result<(), String> {
    ...
    let first = rustdb_base::ids::PageId::from_persisted(1).map_err(|_| "page one")?;
    ...
    let nanos = time(OPERATIONS / 10, |_| {
        ...
        let _ = std::hint::black_box(rustdb_storage::btree::PageLayout::parse(
            pin.bytes(),
            leaf,
            usable,
        ));
