...
//! them from the pinned SQLite binary. Nothing here reads rust-db, so a
//! generated file cannot accidentally record rust-db's own answer as the thing
//! rust-db is graded against - which is the one way a differential corpus can
//! quietly stop testing anything.
//!
//! Usage: `cargo run -p rustdb-compat --bin rustdb-slt`

use std::path::{Path, PathBuf};
use std::process::ExitCode;

use rustdb_compat::oracle::{Driver, Op, TaggedValue};
use rustdb_compat::slt::{Record, TestFile};
...
fn generate(root: &Path) -> Result<usize, String> {
    ...
    for query in corpus_queries(&queries) {
        ...
        let sort = rustdb_compat::slt::sort_mode_for(&query);
        let expected = rustdb_compat::slt::apply_sort(render_rows(&observation.rows, &types), sort);
...
fn render_tagged(value: &TaggedValue, letter: char) -> String {
    match value {
        ...
        TaggedValue::Integer(integer) => match letter {
            'R' => rustdb_compat::slt::format_real(*integer as f64),
            _ => integer.to_string(),
        },
        TaggedValue::Real(real) => match letter {
            'I' => (*real as i64).to_string(),
            _ => rustdb_compat::slt::format_real(*real),
        },
