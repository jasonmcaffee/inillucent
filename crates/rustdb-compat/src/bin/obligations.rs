...
//! What is in them is `rustdb_compat::obligations`, so the harness test can
//! generate the same bytes without shelling out to a program.
//!
//! Usage: `cargo run -p rustdb-compat --bin rustdb-obligations -- [--out <dir>]`

use std::path::PathBuf;
use std::process::ExitCode;

use rustdb_compat::obligations::registers;
