...
//! lands with the phase that qualifies performance; these are rust-db against
...
//!
//! Usage: `cargo run --release -p rustdb-compat --bin rustdb-walperf`
...
fn checkpoint_report(connection: &Connection, mode: CheckpointMode) -> Result<(i64, i64), String> {
    ...
    while prepared.step().map_err(text)? {
        ...
        let field = |index: usize| {
            row.get(index)
                .and_then(rustdb_value::Value::as_integer)
                .unwrap_or(0)
        };
...
fn service_family(scratch: &Path) -> Result<Vec<Measurement>, String> {
    ...
    let started = Instant::now();
    let mut backup =
        rustdb_session::Backup::begin(&connection, 0, &destination, 0).map_err(text)?;
    ...
    let before = Counters::of(&connection);
    let started = Instant::now();
    let bytes = rustdb_session::serialize(&connection, 0).map_err(text)?;
    let total = started.elapsed();
    let counters = Counters::of(&connection).since(before);
    let mut row = measure(
        "services",
    ...
    for key in 0..1_000u64 {
        let at = Instant::now();
        let blob = rustdb_session::Blob::open(&connection, b"main", b"t", b"b", key as i64, true)
            .map_err(text)?;
