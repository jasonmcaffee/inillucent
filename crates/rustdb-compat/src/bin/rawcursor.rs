...
//!
//! Usage: rustdb-rawcursor <database> [rounds]
//!
...
fn main() {
    ...
    let Some(database) = args.next() else {
        eprintln!("usage: rustdb-rawcursor <database> [rounds]");
        std::process::exit(2);
    };
...
fn run(path: &PathBuf, rounds: u32) -> DbResult<()> {
    ...
    let table = find_root(&mut pager, &limits, "table", "main_table")?
        .ok_or_else(|| rustdb_base::error::misuse("no main_table in this database"))?;
    ...
    while more {
        ...
        if kind == wanted_kind.as_bytes() && name == wanted.as_bytes() {
            ...
            let root = u32::try_from(root).map_err(|_| {
                rustdb_base::error::corrupt("a root page that is not a page number")
            })?;
