//! The rust-db side of the oracle protocol.
//!
//! Invariant: this driver answers with what rust-db really does, including
...
//! about rust-db rather than about the harness.

use std::io::{BufRead, Write};

use rustdb_base::buffer;
use rustdb_base::bytes;
use rustdb_base::error::PrimaryCode;
use rustdb_compat::oracle::TaggedValue;
use rustdb_compat::report::json_string;
use rustdb_vfs::contract::{OpenOptions, Vfs};
...
impl Session {
    ...
    fn dispatch(&mut self, command: &str) -> String {
        match operation(command).as_deref() {
            Some("hello") => format!(
                "{{\"ok\":true,\"driver\":\"rustdb\",\"version\":{},\"phase\":{}}}",
                json_string(env!("CARGO_PKG_VERSION")),
                json_string(rustdb_base::IMPLEMENTATION_PHASE)
            ),
...
/// Routes one value through rust-db's own codecs and back.
///
...
fn not_implemented() -> String {
    error_reply(
        PrimaryCode::Error,
        "rust-db has no SQL front end yet; it lands in phase 5",
    )
}
