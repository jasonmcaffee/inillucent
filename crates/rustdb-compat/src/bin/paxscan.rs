...
//! rust-db side too, and the comparison the gate reads is rust-db's best plan
//! against SQLite's best plan, with the like-structure pairs printed beside it.
//!
//! Usage: rustdb-paxscan <sqlite fixture> [rounds] [page size...]
...
use rustdb_sqlite_reader::{borrow, SqliteFile};
...
fn main() {
    ...
    let Some(fixture) = arguments.next() else {
        eprintln!("usage: rustdb-paxscan <sqlite fixture> [rounds] [page size...]");
        std::process::exit(2);
    };
