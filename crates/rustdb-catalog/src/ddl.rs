...
pub fn delete_schema_rows(
    ...
    while more {
        ...
        let record = rustdb_value::record::RecordRef::parse(&payload, pager.text_encoding())?;
...
fn text_of(record: &rustdb_value::record::RecordRef<'_>, index: usize) -> DbResult<Vec<u8>> {
...
fn integer_of(record: &rustdb_value::record::RecordRef<'_>, index: usize) -> DbResult<u32> {
    match record.value(index)? {
