...
use rustdb_sql::ast::Statement;
use rustdb_sql::lexer::{Lexer, Span, TokenKind};
...
pub fn quote_if_needed(name: &[u8]) -> Vec<u8> {
    let plain = name
        ...
        && rustdb_sql::keyword::lookup(&name.to_ascii_lowercase()).is_none();
    if plain {
        return name.to_vec();
    }
...
fn column_list_end(sql: &[u8]) -> Option<usize> {
    ...
    loop {
        ...
        match token.kind {
            ...
            TokenKind::Punctuator(rustdb_sql::lexer::Punctuator::LeftParen) => {
                depth = depth.saturating_add(1);
                opened = true;
            }
            TokenKind::Punctuator(rustdb_sql::lexer::Punctuator::RightParen) => {
...
pub fn drop_column(sql: &[u8], position: usize) -> DbResult<Vec<u8>> {
    ...
    let Statement::CreateTable {
        body: rustdb_sql::ast::CreateTableBody::Columns { columns, .. },
        ..
    } = &parsed.statement
    else {
