...
//! own format, so the pinned build reads statistics rust-db gathered and
//! rust-db reads statistics the pinned build gathered. That is not a nicety.
...
use rustdb_value::{compare, Collation, Value};
...
pub fn clear_stats(pager: &mut Pager, root: u32) -> DbResult<()> {
    ...
    mutate::clear_tree(
        pager,
        PageId::new(root)
            .ok_or_else(|| rustdb_base::error::corrupt("a statistics table with no root"))?,
    )?;
    Ok(())
}
