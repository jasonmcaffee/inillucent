...
use crate::bind::{BoundExpr, BoundSelect};
...
/// Plans a bound SELECT.
///
/// The predicate list is split before any path is chosen, because a path can
/// only consume a term of a conjunction and the rest has to be kept.
pub fn plan_select(select: BoundSelect) -> PhysicalPlan {
    ...
    for (position, source) in select.sources.iter().enumerate() {
        if let Some(constraint) = &source.constraint {
            ...
            split_conjunction(constraint, &mut joined);
            for term in joined {
                terms.push(term);
            }
            let _ = position;
    ...
    for (position, source) in select.sources.iter().enumerate() {
        let path = choose_path(position, &source.table, &terms, &mut consumed);
    ...
    let (residuals, constant_filter) = distribute_residuals(&terms, &consumed, sources.len());
...
fn distribute_residuals(
    ...
    for (index, term) in terms.iter().enumerate() {
        ...
        let level = used.iter().copied().max();
        match level {
            Some(level) if level < levels => {
                let slot = residuals.get_mut(level);
...
fn choose_path(
    ...
    if let Some(path) = rowid_path(position, table, terms, consumed) {
        return path;
    }
    if let Some(path) = index_path(position, table, terms, consumed) {
        return path;
    }
...
fn rowid_path(
    ...
    for (index, term) in terms.iter().enumerate() {
        ...
        let Some((op, value)) = comparison_against_rowid(position, term) else {
            continue;
        };
        if op != BinaryOp::Equal || !is_available(position, &value) {
            continue;
        }
    ...
    for (index, term) in terms.iter().enumerate() {
        ...
        if !is_available(position, &value) {
            continue;
        }
        match op {
...
fn index_path(
    ...
    for index in &table.indexes {
        ...
        let Some((path, used)) = index_candidate(position, table, index, terms, consumed) else {
            continue;
        };
...
fn index_candidate(
    ...
    while let Some(key_column) = index.columns.get(key) {
        ...
        let Some((term_index, value)) =
            find_equality(position, column, collation, terms, consumed, &used)
        else {
            break;
        };
    ...
    if let Some(key_column) = index.columns.get(key) {
        if let Some(column) = key_column.column {
            ...
            for (term_index, term) in terms.iter().enumerate() {
                ...
                let Some((op, value)) = comparison_against_column(position, column, term) else {
                    continue;
                };
                if !is_available(position, &value) || comparison_collation(term) != collation {
                    continue;
                }
                match op {
                    BinaryOp::Greater if low.is_none() => {
...
fn is_available(position: usize, value: &BoundExpr) -> bool {
    let mut used = Vec::new();
    value.sources_used(&mut used);
    used.iter().all(|source| *source < position)
}
...
mod tests {
    ...
    fn a_seek_key_may_only_read_outer_terms() {
        ...
        assert!(is_available(1, &outer));
        assert!(!is_available(0, &outer));
        assert!(is_available(0, &BoundExpr::Integer(5)));
    }
