...
// Every skip here goes through `inillucent_base::testing::skipping`, which is
// the one function in the workspace that prints the `; skipping` marker and
// panics under `INILLUCENT_STRICT`. This file used to define its own copy,
// because the layering contract will not let a production crate reach the
// compat harness; task-1969 §4.6 moved the body to the bottom crate, which is
// below this one, so the copy is gone.
