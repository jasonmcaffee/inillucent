...
//!
//! This crate is `tasks/task-1816-rearchitecture-tdd.md`'s `rustdb-tree`, which
//! replaces `rustdb-storage`. Phase 1 builds the leaf codec and an in-memory
...
//! moves the same leaf codec onto `rustdb-pool` frames; nothing in
...
#![deny(clippy::panic)]
// `clippy::arithmetic_side_effects` is denied in `rustdb-base` and not here,
// which is the same line `rustdb-storage` draws. A crate that reads pages needs
