...
//! rules live in `rustdb_value` where the engine already applies them; this
...
fn text_of(value: &Value<'_>) -> Vec<u8> {
    let cast = rustdb::cast::cast_value(
        value.clone(),
        rustdb::Affinity::Text,
        rustdb::TextEncoding::Utf8,
    );
    match cast {
...
pub unsafe extern "C" fn sqlite3_result_error(
    ...
    held.result = Err(DbError::primary(rustdb::PrimaryCode::Error).with_message(text));
}
...
pub unsafe extern "C" fn sqlite3_result_error_code(context: *mut sqlite3_context, code: c_int) {
    ...
    held.result = Err(DbError::new(rustdb::ExtendedCode(code)).with_message(message));
}
