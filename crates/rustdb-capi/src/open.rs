...
pub unsafe extern "C" fn sqlite3_open_v2(
    ...
    *out = std::ptr::null_mut();
    ...
    let mut options = rustdb::ConnectionOptions::default();
...
fn build(
    path: &str,
    options: rustdb::ConnectionOptions,
    ...
    match open_pair(path, options) {
        ...
        Err(error) => {
            ...
            match open_pair(":memory:", rustdb::ConnectionOptions::default()) {
...
fn assemble(database: Database, connection: rustdb::Connection, last: ErrorSlot) -> Box<sqlite3> {
    Box::new(sqlite3 {
...
fn build_on(
    path: &str,
    options: rustdb::ConnectionOptions,
    file_system: std::sync::Arc<dyn rustdb::vfs::Vfs>,
) -> Result<Box<sqlite3>, (Option<Box<sqlite3>>, c_int)> {
...
fn open_pair(
    path: &str,
    options: rustdb::ConnectionOptions,
) -> Result<(Database, rustdb::Connection), DbError> {
...
fn names(database: &sqlite3) -> Vec<Vec<u8>> {
    ...
    rows.iter()
        ...
        .filter_map(rustdb::Value::as_text)
