...
pub struct sqlite3_backup {
    ...
    inner: Option<rustdb::Backup<'static>>,
    /// What the last step reported, for `remaining` and `pagecount`.
    progress: rustdb::BackupProgress,
}
...
pub unsafe extern "C" fn sqlite3_backup_init(
    ...
    let source_connection: &'static rustdb::Connection = std::mem::transmute(&*from.connection);
    let destination_connection: &'static rustdb::Connection = std::mem::transmute(&*to.connection);
