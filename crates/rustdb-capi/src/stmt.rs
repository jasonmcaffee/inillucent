...
pub unsafe extern "C" fn sqlite3_prepare_v3(
    ...
    *out = std::ptr::null_mut();
    ...
    let owner: &'static rustdb::Connection = std::mem::transmute(&*database.connection);
    match owner.prepare_with_tail(&text) {
        ...
        Ok((prepared, _consumed)) => {
            ...
            let held = Box::new(sqlite3_stmt {
                ...
                values: Vec::new(),
                sql: sql_text,
                bound: vec![rustdb::Value::Null; parameters],
            ...
            Ok(true) => {
                ...
                let values: Vec<*mut c_char> = prepared
                    ...
                    .map(|value| match value {
                        rustdb::Value::Null => std::ptr::null_mut(),
