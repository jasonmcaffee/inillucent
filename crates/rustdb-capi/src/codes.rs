...
//! `compat/errors.toml` into `rustdb_base`, which is where the engine already
//! keeps them, and this module re-exports what the boundary needs.

use rustdb::{DbError, ExtendedCode, PrimaryCode};
...
pub const SQLITE_SOURCE_ID: &str = "rust-db (SQLite 3.53.4 ABI)";
