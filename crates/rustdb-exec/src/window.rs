...
//! frame contains comes from [`rustdb_scalar::window`], which the bytecode VM
...
use rustdb_sql::function::{AggregateFunc, WindowFunc};
...
fn integer_of(value: &Datum<'_>) -> i64 {
    match value {
        ...
        Datum::Null => 0,
        other => rustdb_value::cast::integer_value(&crate::scalar::to_value(*other)),
    }
}
...
fn real_of(value: &Datum<'_>) -> f64 {
    match value {
        ...
        Datum::Null => 0.0,
        other => rustdb_value::cast::real_value(&crate::scalar::to_value(*other)),
    }
}
...
///
/// The window binder names its aggregates with `rustdb_sql`'s enum and the
