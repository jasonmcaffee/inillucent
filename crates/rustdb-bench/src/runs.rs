...
mod tests {
    ...
    fn a_password_is_removed_from_a_connection_string() {
        assert_eq!(
            redact("postgres://postgres:hunter2@127.0.0.1:5433/rustdb_synth"),
            "postgres://postgres:***@127.0.0.1:5433/rustdb_synth"
        );
    }
    ...
    fn a_connection_string_without_a_password_is_left_alone() {
        assert_eq!(
            redact("postgres://127.0.0.1:5433/rustdb_synth"),
            "postgres://127.0.0.1:5433/rustdb_synth"
        );
    ...
    fn a_run_writes_its_records_and_then_its_manifest() {
        let root = std::env::temp_dir().join(format!("rustdb-runs-{}", std::process::id()));
        ...
        let manifest = RunManifest {
            ...
            git_commit: "abc".into(),
            git_dirty: false,
            command: "rustdb-bench grade".into(),
            corpus: CorpusFacts {
                chunks: 1,
                documents: 1,
                dimensions: 768,
        ...
        assert_eq!(parsed["engine"], "rust-db");
