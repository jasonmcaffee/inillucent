...
//! the same Reciprocal Rank Fusion constants rust-db uses, so the baseline is a
...
//! rust-db by comparing against a misconfiguration.
//!
...
use rustdb_core::index::Index;
use rustdb_core::rank::{self, Fusion, PER_DOC_CAP};
...
impl PgVectorEngine {
    ...
    /// positionally starting at `$start`. The same predicates rust-db compiles, so
...
/// rust-db behind the same trait. Chunk ordinals are mapped back to the source
/// database identifiers so hits from the two engines are directly comparable.
pub struct RustDbEngine {
    pub index: Index,
    pub keys: Vec<String>,
    pub name: String,
...
impl SearchEngine for RustDbEngine {
    fn name(&self) -> &str {
        &self.name
    }
...
/// correct answer for every vector accuracy scenario. Computed inside rust-db
