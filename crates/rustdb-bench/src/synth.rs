...
pub fn default_derived_dir() -> PathBuf {
    ...
    PathBuf::from(home).join(".cache/rust-db-corpus/derived")
}
...
pub fn embed(
    ...
    let dims = rustdb_core::embed::NOMIC_DIMS;
...
pub fn load_postgres(
    ...
    let mut client = Client::connect(url, NoTls).with_context(|| {
        format!("connecting to {url}. Create the database first: createdb rustdb_synth")
    })?;
