...
pub fn judge(card: &ScoreCard) -> Vec<Judgement> {
    ...
    for sc in &card.scenarios {
        for row in &sc.rows {
            ...
            let paired = match (row.values_for("rust-db"), row.values_for(&best.engine)) {
...
pub fn render(card: &ScoreCard) -> String {
    ...
    if !judgements.is_empty() {
        ...
        s.push_str("`delta` is rust-db minus the baseline, oriented so positive is better whatever the metric's own direction. The interval is the 95% paired bootstrap on that delta; `p` is the paired randomization test. `n` is the queries behind it and `moved` is how many of them the two engines answered differently — a comparison resting on three queries is worth reading with suspicion however small its p-value.\n\n");
        s.push_str("| family | measurement | metric | rust-db | baseline | delta | 95% interval | p | n | moved | threshold | verdict |\n");
        s.push_str("|---|---|---|---|---|---|---|---|---|---|---|---|\n");
        for j in &judgements {
            let (delta, interval, pv, n, moved) = match &j.paired {
                ...
                None => {
                    let d = if j.rustdb.is_finite() { j.rustdb - j.best_baseline } else { 0.0 };
...
pub fn print_summary(card: &ScoreCard) {
    ...
    for x in j.iter().filter(|x| x.verdict != Verdict::Better) {
        ...
        eprintln!(
            ...
            fmt(x.rustdb), fmt(x.best_baseline), x.best_baseline_engine, interval
        );
    }
    eprintln!("\n=== summary ===");
    for sc in &card.scenarios {
...
mod tests {
    ...
    fn card_with(rustdb: &[f64], baseline: &[f64]) -> ScoreCard {
        ...
        ScoreCard {
            ...
            scenarios: vec![Scenario {
                ...
                rows: vec![MetricRow::primary(
                    ...
                    "recall@10",
                    vec![
                        Measure { engine: "rust-db".into(), value: mean(rustdb) },
                        Measure { engine: "pgvector".into(), value: mean(baseline) },
                    ],
                    true,
                    vec![
                        Series { engine: "rust-db".into(), values: rustdb.to_vec() },
    ...
    fn the_verdict_counts_a_better_result_against_the_better_baseline() {
        ...
        c.scenarios[0].rows[0].series = vec![
            Series { engine: "rust-db".into(), values: vec![0.95; 40] },
    ...
    fn a_worse_result_against_the_better_baseline_is_reported() {
        ...
        c.scenarios[0].rows[0].series = vec![
            Series { engine: "rust-db".into(), values: vec![0.80; 40] },
            Series { engine: PgMode::Default.label().into(), values: vec![0.10; 40] },
            Series { engine: PgMode::WellConfigured.label().into(), values: vec![0.90; 40] },
        ];
        let j = judge(&c);
    ...
    fn one_query_of_ninety_is_no_longer_a_win() {
        ...
        let mut rustdb = baseline.clone();
        rustdb[0] = 1.0;
        let c = card_with(&rustdb, &baseline);
        let j = judge(&c);
        assert!(j[0].rustdb > j[0].best_baseline, "the point estimate really did move");
    ...
    fn a_diagnostic_row_is_rendered_but_not_judged() {
        ...
        c.scenarios[0].rows.push(MetricRow::diagnostic(
            ...
            "rows returned of 50",
            vec![
                Measure { engine: "rust-db".into(), value: 50.0 },
    ...
    fn a_lower_is_better_row_is_judged_in_the_right_direction() {
        ...
        c.scenarios[0].rows[0].series = vec![
            Series { engine: "rust-db".into(), values: vec![1.2; 40] },
