...
use rustdb_sql::dml::{BoundDelete, BoundInsert, BoundInsertSource, BoundUpdate, ColumnSource};
use rustdb_sql::vtab::IndexQuery;
...
impl Compiler {
    ...
    pub(crate) fn open_virtual_loop(
        ...
        let terms = filter
            .map(rustdb_sql::plan::conjunction)
            .unwrap_or_default();
        let offer = rustdb_sql::plan::virtual_offer(source, table, &terms);
