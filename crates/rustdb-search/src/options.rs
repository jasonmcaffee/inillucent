//! What a `CREATE VIRTUAL TABLE ... USING rustdb_search(...)` said, and what
...
//! are compared against the pinned release. `rustdb_search` is not a parity
...
use rustdb_ext::vtab::failure;

/// The tokenizer this build has, named so a stored table records which one
/// produced its terms.
///
/// One name, not a family: `rustdb-core` has exactly one tokenizer - Porter
...
/// the same rule `rustdb_core::persist` applies to a generation directory and
    ...
    pub fn parse(text: &str) -> DbResult<Mode> {
        match text.trim().to_ascii_lowercase().as_str() {
            ...
            other => Err(failure(format!(
                "rustdb_search: mode must be exact or approximate, not {other}"
            ))),
        }
    }
}
...
impl Metric {
    ...
    pub fn parse(text: &str) -> DbResult<Metric> {
        match text.trim().to_ascii_lowercase().as_str() {
            ...
            other => Err(failure(format!(
                "rustdb_search: the only distance this build implements is cosine, not {other}"
            ))),
        }
    }
}
...
/// Reads the arguments of a `CREATE VIRTUAL TABLE ... USING rustdb_search(...)`.
///
        ...
        match name.to_ascii_lowercase().as_str() {
            "dims" | "dimensions" => {
                dims = value.parse::<usize>().map_err(|_| {
                    failure(format!("rustdb_search: dims must be a number, not {value}"))
                })?;
                if dims > MAX_DIMS {
                    return Err(failure(format!(
                        "rustdb_search: dims must be at most {MAX_DIMS}"
                    )));
                }
            }
            ...
            "tokenize" | "tokenizer" => {
                if !value.eq_ignore_ascii_case(TOKENIZER) {
                    return Err(failure(format!(
                        "rustdb_search: the only tokenizer this build has is {TOKENIZER}, not {value}"
                    )));
                }
            }
            "compact" => {
                compact = Some(value.parse::<u64>().map_err(|_| {
                    failure(format!(
                        "rustdb_search: compact must be a number, not {value}"
                    ))
                })?);
            }
            other => return Err(failure(format!("rustdb_search: no such option: {other}"))),
        }
    }
    if columns.is_empty() {
        return Err(failure(
            "rustdb_search: a search table needs at least one column",
        ));
    }
    Ok(Options {
        columns,
...
pub fn from_config(rows: &[(String, String)], fallback: &Options) -> DbResult<Options> {
    ...
    if let Some(format) = find("format") {
        if format.trim() != FORMAT.to_string() {
            return Err(failure(format!(
                "rustdb_search: the table was written in format {format}, this build reads format {FORMAT}"
            )));
        }
    }
    ...
    if let Some(tokenizer) = find("tokenize") {
        if !tokenizer.eq_ignore_ascii_case(TOKENIZER) {
            return Err(failure(format!(
                "rustdb_search: the terms were produced by {tokenizer}, which this build does not have"
            )));
        }
    }
