...
//! wrong. `crates/rustdb-search/src/store.rs` is where that decision lives.
//!
//! ## What this is not
//!
...
//! value `rustdb-core` ships, because the quality scorecard that justifies them
//! was produced with them.
//!
//! ## The three declarations
//!
//! A retrieval engine that will not say what it is doing cannot be checked, so
//! `rustdb_search` declares, in the table's own `%_config` rows:
//!
...
pub fn register(registry: &mut rustdb_ext::registry::Registry) {
...
mod tests {
    ...
    fn registering_makes_the_module_findable() {
        let mut registry = rustdb_ext::registry::Registry::default();
        register(&mut registry);
        assert!(registry.module(b"rustdb_search").is_some());
        assert!(registry.module(b"RUSTDB_SEARCH").is_some());
    }
}
