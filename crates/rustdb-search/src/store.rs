...
//!   each a `rustdb_core::persist` byte stream cut into rows. A generation is
...
use rustdb_ext::shadow::ShadowTables;
use rustdb_ext::vtab::{failure, Context, ModuleArguments, ShadowTable};
...
///
/// Little-endian `f32`, which is the layout `rustdb_core` keeps vectors in and
...
pub fn vector_of(value: &Value<'static>, dims: usize) -> DbResult<Vec<f32>> {
    let vector = match value {
        ...
        _ => {
            return Err(failure(
                "rustdb_search: a vector is a blob of little-endian 32-bit floats",
            ))
        }
    };
    if vector.len() != dims {
        return Err(failure(format!(
            "rustdb_search: this index has {dims} dimensions, and the vector has {}",
            vector.len()
        )));
    }
    Ok(vector)
