//! The `rustdb_search` virtual table: its declaration, its access path, its
...
//!
//! ```sql
//! CREATE VIRTUAL TABLE docs USING rustdb_search(
...
impl SearchTable {
            ...
            Some(value) => {
                if !self.options.has_vectors() {
                    return Err(constraint(
                        "rustdb_search: this table was declared without a vector width",
                    ));
                }
    ...
    fn command(&mut self, context: &mut Context<'_>, command: &str) -> DbResult<()> {
        match command.trim().to_ascii_lowercase().as_str() {
            ...
            other => Err(failure(format!("rustdb_search: no such command: {other}"))),
        }
    }
    ...
    fn compact(&mut self, context: &mut Context<'_>, force: bool) -> DbResult<()> {
        ...
        rustdb_core::persist::write_index(&index, &mut bytes).map_err(|error| {
            failure(format!("rustdb_search: cannot write a generation: {error}"))
        })?;
        let generation = self
            .store
    ...
    fn integrity(&mut self, context: &mut Context<'_>) -> DbResult<Option<String>> {
        ...
        if generation > 0 {
            match self.store.read_generation(context, generation)? {
                ...
                Some(bytes) => {
                    if let Err(error) = rustdb_core::persist::read_index(&mut bytes.as_slice()) {
...
fn origin_name(origin: rustdb_core::rank::HitOrigin) -> &'static str {
    match origin {
        rustdb_core::rank::HitOrigin::Vector => "vector",
        rustdb_core::rank::HitOrigin::Lexical => "lexical",
        rustdb_core::rank::HitOrigin::Both => "both",
    }
}
...
mod tests {
    ...
    #[test]
    fn the_module_is_named_rustdb_search() {
        assert_eq!(SearchModule.name(), "rustdb_search");
