...
use rustdb_migrate::copy;
use rustdb_migrate::index::SqlIndex;
...
fn chunk(ordinal: usize) -> ChunkInput {
    let text = (0..4)
        ...
        .join(" ");
    ChunkInput {
        source: "rustdb_search".to_string(),
...
fn open_sql(root: &Path, dims: usize) -> SqlIndex {
    ...
    {
        let database = rustdb::Database::open(&path).expect("the database opens");
...
fn a_tombstone_removes_the_same_row_from_both() {
    ...
    assert!(RetrievalIndex::tombstone(&mut direct, "rustdb_search", "7").expect("tombstoned"));
    assert!(RetrievalIndex::tombstone(&mut sql, "rustdb_search", "7").expect("tombstoned"));
    assert!(!RetrievalIndex::tombstone(&mut sql, "rustdb_search", "999").expect("absent"));
