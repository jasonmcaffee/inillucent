...
use rustdb_core::distance::normalize;
use rustdb_core::index::{Index, IndexConfig};
use rustdb_core::store::ChunkInput;
use rustdb_migrate::manifest::Manifest;
use rustdb_migrate::{migrate, Plan};
...
fn build_source(directory: &Path, documents: usize, chunks_each: u32) -> Index {
    ...
    index.commit();
    rustdb_core::persist::save(&index, directory).expect("the legacy index saves");
    index
}
...
fn a_legacy_index_migrates_and_every_check_passes() {
    ...
    assert!(
        rustdb_core::persist::load(&source_dir).is_ok(),
...
fn an_interrupted_migration_resumes() {
    ...
    // manifest header, the schema, and one committed batch of documents.
    {
        ...
        let inventory =
            rustdb_migrate::source::Source::open(&source_dir).expect("the source opens");
        ...
        let database = rustdb::Database::open(&plan.staging).expect("the staging opens");
        let connection = database.connect().expect("it connects");
        rustdb_migrate::copy::create_schema(&connection, DIMS).expect("the schema builds");
        manifest.record("stage", "schema").expect("recorded");
        rustdb_migrate::copy::copy_documents(&connection, source.store(), &mut manifest)
...
fn rolling_back_is_opening_the_source() {
    ...
    let reopened = rustdb_core::persist::load(&source_dir).expect("the source reopens");
    assert_eq!(reopened.store().n_chunks(), expected);
    let filter = reopened.compile(&rustdb_core::filter::Filter::default());
...
fn a_lexical_only_source_migrates() {
    ...
    index.add(inputs, &vectors);
    index.commit();
    rustdb_core::persist::save(&index, &source_dir).expect("the legacy index saves");
