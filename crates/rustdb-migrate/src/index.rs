...
//!
//! Invariant: this answers the same questions as `rustdb_core::index::Index`
//! and answers them the same way. It is the second implementation of
//! `rustdb_search::adapter::RetrievalIndex`; the first is the direct engine
...
//! the same `rustdb-core` code the direct engine runs, because the search
//! table's base generation *is* a `rustdb-core` index - it is written by
...
use rustdb::{Connection, Database, DbError, DbResult, Value};
use rustdb_core::rank::HitOrigin;
use rustdb_core::store::ChunkInput;
use rustdb_search::adapter::{Hit, Query, RetrievalIndex};
...
impl SqlIndex {
    ...
    /// @param table - the `rustdb_search` table's name
...
fn declared_dims(connection: &Connection, table: &str) -> DbResult<usize> {
    ...
    if !statement.step()? {
        return Err(DbError::primary(rustdb_base::PrimaryCode::Error)
            .with_detail(format!("{table} is not a rustdb_search table")));
    }
...
impl RetrievalIndex for SqlIndex {
    ...
    fn append(&mut self, chunks: Vec<ChunkInput>, embeddings: &[Vec<f32>]) -> DbResult<usize> {
        if chunks.len() != embeddings.len() {
            return Err(DbError::primary(rustdb_base::PrimaryCode::Misuse)
