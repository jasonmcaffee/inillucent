//! Copy a legacy rust-db index into a rust-db database, verify it, and publish
...
pub fn migrate(plan: &Plan) -> Result<Outcome, String> {
    ...
    if !resuming {
        ...
        manifest.record("tool", "rustdb-migrate")?;
...
fn scalar_text(connection: &rustdb::Connection, sql: &str) -> Result<String, String> {
    ...
    Ok(match statement.row().first() {
        Some(rustdb::Value::Integer(number)) => number.to_string(),
        Some(rustdb::Value::Text(text)) => String::from_utf8_lossy(&text.utf8_bytes()).into_owned(),
        _ => String::new(),
    })
}
