...
//! loads the index through `rustdb_core::persist::load` - the reader the
...
impl Source {
    ...
    pub fn open(path: impl AsRef<Path>) -> Result<Source, String> {
        ...
        let index = rustdb_core::persist::load(&path)
...
mod tests {
    ...
    fn a_directory_with_no_pointer_is_refused() {
        ...
        path.push(format!("rustdb-migrate-empty-{}", std::process::id()));
