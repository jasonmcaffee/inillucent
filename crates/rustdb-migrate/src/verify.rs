...
//! by the same `rustdb-core` code from the same text: one column holding the
...
use rustdb::Connection;
use rustdb_base::hash::Sha256;
use rustdb_core::filter::Filter;
use rustdb_core::index::{Branches, Index};
use rustdb_core::store::Store;
use rustdb_search::adapter::{Query, RetrievalIndex};
...
/// chunk belongs to. A `rustdb_search` table has one row per document by
...
fn retrieval(source: &Index, sql: &mut SqlIndex) -> Vec<Check> {
    let store = source.store();
    if store.n_chunks() == 0 {
        return vec![Check::pass("retrieval", "the corpus is empty")];
    }
...
fn scores_match(
    ...
    if wrong.is_empty() {
        ...
        Check::fail("bm25.scores", wrong.join(" | "))
    }
}
...
fn live_filter(
    ...
    sql: &mut SqlIndex,
    probes: &[String],
    live: &rustdb_core::filter::CompiledFilter,
    documents: &[(i64, i64)],
    deleted: &[i64],
    cap: usize,
    wide: usize,
    ...
    if deleted.is_empty() {
        return Check::pass("filter.deleted", "the corpus holds no tombstoned chunk");
    }
    let mut wrong = Vec::new();
...
fn chunk_documents(connection: &Connection) -> Result<Vec<(i64, i64)>, String> {
        ...
        .map_err(|error| error.message().to_string())?
    {
        ...
        if let (Some(chunk), Some(document)) = (
            row.first().and_then(rustdb::Value::as_integer),
            row.get(1).and_then(rustdb::Value::as_integer),
        ) {
...
fn scalar(connection: &Connection, sql: &str) -> Result<i64, String> {
    ...
    Ok(statement
        .row()
        .first()
        .and_then(rustdb::Value::as_integer)
        .unwrap_or(0))
}
...
fn integers(connection: &Connection, sql: &str) -> Result<Vec<i64>, String> {
    ...
    while statement
        ...
        .map_err(|error| format!("{sql}: {}", error.message()))?
    {
        if let Some(value) = statement.row().first().and_then(rustdb::Value::as_integer) {
            rows.push(value);
        }
    }
    Ok(rows)
...
fn strings(connection: &Connection, sql: &str) -> Result<Vec<String>, String> {
    ...
    while statement
        ...
        .map_err(|error| format!("{sql}: {}", error.message()))?
    {
        if let Some(rustdb::Value::Text(value)) = statement.row().first() {
