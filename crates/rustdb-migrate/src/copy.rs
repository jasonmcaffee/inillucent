...
use rustdb::{Connection, DbResult, Value};
...
pub fn schema(dims: usize) -> Vec<String> {
    ...
    statements.push(format!(
        "CREATE VIRTUAL TABLE {SEARCH_TABLE} USING rustdb_search(content, dims = {dims}, \
...
pub fn copy_search(
    connection: &Connection,
    store: &Store,
    vectors: &rustdb_core::vectors::VectorSet,
    dims: usize,
    manifest: &mut Manifest,
) -> Result<u64, String> {
...
fn bind_text(statement: &mut rustdb::Statement<'_>, index: u32, value: &str) -> Result<(), String> {
    statement
...
fn drain(statement: &mut rustdb::Statement<'_>) -> Result<(), String> {
    while statement
        .step()
        .map_err(|error| error.message().to_string())?
    {}
...
mod tests {
    ...
    fn the_search_table_has_one_column() {
        ...
        let search = statements
            .iter()
            .find(|statement| statement.contains("rustdb_search"))
