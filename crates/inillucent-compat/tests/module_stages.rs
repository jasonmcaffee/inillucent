...
fn no_stage_is_recorded_until_a_harness_asks() {
    ...
    assert_eq!(
        ...
        "the virtual table stage counters recorded {DOCUMENTS} documents nobody asked to be \
...
fn the_recorded_stages_nest_the_way_the_calls_do() {
    ...
    assert_eq!(
        ...
        "the insert arm reported {} rows for {DOCUMENTS} inserts",
        stages.rows
    );
...
fn the_module_is_the_majority_of_what_the_insert_arm_costs() {
    let mut database = database("majority");
    database.record_module_stages(true);
    fill_documents(&mut database);
    let stages = database.module_stage_nanos();
    database.record_module_stages(false);

    assert!(stages.whole > 0, "the insert arm was not timed at all");
    assert!(
...
fn switching_the_timing_off_stops_the_counters() {
    ...
    assert_eq!(timed.rows, DOCUMENTS as u64, "the first pass was not timed");
...
fn an_rtree_insert_is_counted_by_the_same_counters() {
    let mut database = database("rtree");
    database.record_module_stages(true);
    ...
    assert_eq!(
        stages.rows, DOCUMENTS as u64,
        "the insert arm reported {} rows for {DOCUMENTS} R-Tree inserts",
        stages.rows
    );
    assert!(
