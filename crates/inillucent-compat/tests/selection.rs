...
fn calls_the_skip_helper(file: &std::path::Path) -> bool {
    ...
    // `cliproc::program` counted here as a skip until task-2106, and every
    // suite that ran a program declared `programs` for it. It now panics with
    // cargo's output when the build fails, because a program built from this
    // workspace is not something a machine can be missing.
