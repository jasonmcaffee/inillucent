...
use rustdb_sim::schedule::{ActorId, Decisions, Scheduler};
use rustdb_sim::sim_vfs::{set_current_actor, SimConfig, SimVfs};
use rustdb_vfs::conformance;
use rustdb_vfs::contract::{FileLock, OpenOptions, SyncMode, Vfs};
...
fn crash_artifacts_are_written_to_disk() {
    ...
    directory.push(format!("rustdb-sim-artifacts-{}", std::process::id()));
