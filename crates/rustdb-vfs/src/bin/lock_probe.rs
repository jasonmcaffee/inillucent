...
use rustdb_vfs::contract::{FileLock, OpenOptions, Vfs, VfsFile};
use rustdb_vfs::error::VfsError;
...
fn main() {
    ...
    for line in stdin.lock().lines() {
        ...
        let reply = dispatch(&vfs, &mut file, command);
...
fn dispatch(vfs: &OsVfs, file: &mut Option<Box<dyn VfsFile>>, command: &str) -> String {
    ...
    F: FnOnce(&dyn VfsFile) -> Option<Result<(), VfsError>>,
{
    ...
    match action(handle.as_ref()) {
        ...
        Some(Err(error)) if error.code() == rustdb_base::error::PrimaryCode::Busy => {
            "busy".to_string()
        }
