...
//! `rustdb-sim` and the compatibility harness can run it against a VFS they
...
fn short_read_zero_fills(vfs: &dyn Vfs, root: &DbPath) -> Result<(), String> {
    ...
    match file.read_exact_at(0, &mut buffer) {
        ...
        Err(error) => {
            if error.extended() != rustdb_base::error::ExtendedCode::IO_ERR_SHORT_READ {
...
fn temp_path_is_fresh(vfs: &dyn Vfs, _root: &DbPath) -> Result<(), String> {
    ...
    for _ in 0..8 {
        let path = describe("temp", vfs.temp_path("rustdb-conformance-"))?;
        if describe("access", vfs.access(&path, AccessMode::Exists))? {
...
fn delete_on_close_removes_the_file(vfs: &dyn Vfs, _root: &DbPath) -> Result<(), String> {
    let path = describe("temp", vfs.temp_path("rustdb-transient-"))?;
    let file = describe(
        "open",
