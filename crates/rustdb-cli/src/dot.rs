...
fn databases(shell: &mut Shell) {
    ...
    for row in rows {
        ...
        shell.say(&format!("{name}: {file}"));
    }
}

/// Returns a value as plain text.
fn text_of(value: Option<&rustdb::Value<'static>>) -> String {
    match value {
        Some(rustdb::Value::Text(text)) => String::from_utf8_lossy(text.raw()).into_owned(),
        Some(rustdb::Value::Null) | None => String::new(),
...
fn tables(shell: &mut Shell, arguments: &[&str]) {
    ...
    // SQLite's shell prints them in columns across the terminal; this prints
    // one line, which is the same information and is what a script wants.
    let line = names.join("  ");
    shell.say(&line);
}
...
fn indexes(shell: &mut Shell, arguments: &[&str]) {
    ...
    if let Some(table) = arguments.first() {
        sql.push_str(&format!(" AND tbl_name = {}", literal_text(table)));
    }
...
fn backup(shell: &mut Shell, arguments: &[&str]) {
    ...
    let outcome = rustdb::Database::open(path)
...
fn version(shell: &mut Shell) {
    ...
    shell.say("rust-db (a first-party engine implementing the SQLite ABI)");
}
