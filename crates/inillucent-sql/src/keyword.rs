...
//!
//! SQLite's real rule is per grammar position: a `%fallback` declaration in its
//! grammar lets a large set of keywords stand in for an identifier wherever an
//! identifier is expected. That set is reproduced here as
//! [`Keyword::may_fall_back`], and the parser asks the question at each
//! position rather than deciding once at lex time.
