...
use rustdb_base::error::{corrupt, misuse, DbError};
use rustdb_base::page::PageSize;
use rustdb_base::{DbResult, PrimaryCode};
use rustdb_storage::wal::{
    CheckpointMode, CheckpointOutcome, WalSnapshot, WalStats, WriteAheadLog,
};
use rustdb_vfs::{AccessMode, DbPath, FileKind, OpenOptions, SyncMode, Vfs, VfsFile};
...
pub struct Wal {
    ...
    append_checksum: rustdb_base::checksum::WalChecksum,
...
impl Wal {
    ...
    pub fn open(
        ...
        Ok(Wal {
            ...
            min_frame: 0,
            append_frame: 0,
            append_checksum: rustdb_base::checksum::WalChecksum::default(),
            exclusive: false,
            options,
    ...
    fn recover_locked(&mut self) -> DbResult<()> {
        ...
        self.header = IndexHeader {
            ...
            initialised: true,
            big_endian_checksum: matches!(log.order, rustdb_base::checksum::WalByteOrder::Big),
            page_size_encoded: log.page_size.to_encoded(),
            max_frame: scan.max_frame,
            page_count: scan.page_count,
            frame_checksum: scan.checksum,
            salt: log.salt,
            checksum: rustdb_base::checksum::WalChecksum::default(),
        };
    ...
    fn put_log_header(&mut self, header: WalHeader) -> DbResult<WalHeader> {
        ...
        self.header.big_endian_checksum =
            matches!(header.order, rustdb_base::checksum::WalByteOrder::Big);
    ...
    fn write_frame(
        ...
        log: &WalHeader,
    ) -> DbResult<rustdb_base::checksum::WalChecksum> {
    ...
    /// database. That is SQLite's documented trade and rust-db makes the same
...
struct LogScan {
    max_frame: u32,
    page_count: u32,
    checksum: rustdb_base::checksum::WalChecksum,
}

impl LogScan {
    /// Returns the result of scanning a log with no valid frames.
    fn empty(checksum: rustdb_base::checksum::WalChecksum) -> LogScan {
        LogScan {
            max_frame: 0,
            page_count: 0,
            checksum,
...
impl WriteAheadLog for Wal {
    ...
    fn begin_write(&mut self) -> DbResult<()> {
        ...
        if published != Some(self.header) {
            ...
            return Err(DbError::new(rustdb_base::ExtendedCode(517))
                .with_message("database is locked")
