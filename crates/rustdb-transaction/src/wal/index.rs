...
//! what `rustdb_vfs::os::ranges::SHM_LOCK_FIRST` already names.
//!
//! Reference: <https://sqlite.org/walformat.html#the_wal_index_file_format>.

use std::sync::Arc;

use rustdb_base::checksum::{WalByteOrder, WalChecksum};
use rustdb_base::error::{corrupt, misuse};
use rustdb_base::page::PageSize;
use rustdb_base::{bytes, DbResult};
use rustdb_vfs::{SharedMemory, ShmLockRequest, ShmRegion};
...
/// The index version rust-db writes and is willing to read.
...
mod tests {
    use super::*;
    use rustdb_vfs::{MemoryVfs, OpenOptions, Vfs};

    /// Opens an index over a memory-backed shared-memory file.
    fn index() -> WalIndex {
        let vfs = MemoryVfs::new();
        let path = rustdb_vfs::DbPath::new(std::path::PathBuf::from("/wal-index.db"));
