...
use rustdb_vfs::{SyncMode, VfsFile};
...
fn copy_frames(wal: &mut Wal, database: &dyn VfsFile, after: u32, safe: u32) -> DbResult<()> {
    ...
    for (page, frame) in newest {
        ...
        let Ok(page_id) = rustdb_base::ids::PageId::from_persisted(page) else {
            continue;
        };
...
mod tests {
    ...
    use rustdb_storage::wal::WriteAheadLog;
    use rustdb_vfs::{DbPath, MemoryVfs, OpenOptions, Vfs};
    ...
    fn a_truncating_checkpoint_empties_the_log() {
        ...
        let log = vfs
            .open(
                ...
                OpenOptions::of_kind(rustdb_vfs::FileKind::Wal),
            )
            .unwrap();
