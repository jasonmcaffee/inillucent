...
//! "port into `rustdb-exec`". Copying them would have produced two `substr()`s
//! that agree today and disagree after the next fix; this crate is that port,
//! done as a move.
//!
//! `rustdb-vm`'s `eval`, `builtin`, `pattern`, `mathfn`, `printf` and
...
//!
//! ## Why it sits above `rustdb-sql`
//!
//! Because a built-in is identified by `rustdb_sql::function::ScalarFunc`, and
