...
use rustdb_value::{Collation, Value};

pub use rustdb_sql::vtab::{
...
pub fn read_only() -> DbError {
    DbError::primary(rustdb_base::PrimaryCode::Error).with_detail("table may not be modified")
}

/// Returns a statement error in a module's own words.
pub fn failure(detail: impl Into<String>) -> DbError {
    DbError::primary(rustdb_base::PrimaryCode::Error).with_detail(detail)
}

/// Returns a constraint failure, which is what a rejected row reports.
pub fn constraint(detail: impl Into<String>) -> DbError {
    DbError::primary(rustdb_base::PrimaryCode::Constraint).with_detail(detail)
}
...
mod tests {
    ...
    fn a_declared_column_carries_its_affinity() {
        ...
        assert_eq!(column.affinity, rustdb_value::Affinity::Integer);
    ...
    fn a_read_only_module_refuses_a_write() {
        assert_eq!(read_only().code(), rustdb_base::PrimaryCode::Error);
    }
}
