...
use rustdb_sql::function::JsonFunc;
use rustdb_value::{numeric, Value};
...
pub fn malformed() -> rustdb_base::DbError {
    failure("malformed JSON")
}

/// Returns a statement error, which is what every JSON refusal is.
pub fn failure(detail: impl Into<String>) -> rustdb_base::DbError {
    rustdb_base::DbError::primary(rustdb_base::PrimaryCode::Error).with_detail(detail)
}
