...
//!
//! - **The correctness gate.** The differential harness compares rust-db against
//!   SQLite 3.53.4 executing the same SQL on the same *logical* data. With the
//!   shared file gone, the rust-db side gets its data by importing the SQLite
//!   fixture through this reader. Same rows, different bytes.
//! - **Migration.** Every database the previous tickets produced is in SQLite
//!   format, and `rustdb-migrate` has to be able to read one.
//!
//! It is the read half of `rustdb-storage` with a narrow interface in front of
...
//! `rustdb-storage`'s pager and b-tree cursor rather than duplicating them, so
...
use rustdb_value::record::{FieldSpan, KeyInfo, RecordRef};
use rustdb_value::{TextEncoding, Value};
use rustdb_vfs::path::DbPath;
use rustdb_vfs::{OsVfs, Vfs};
...
impl SchemaObject {
    ...
    /// for the tables `rustdb-migrate` has to read, and it refuses rather than
    ...
    ///
    /// A general answer needs the real parser in `rustdb-sql`, and this crate
