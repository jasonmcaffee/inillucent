...
/// The largest serial type rust-db will decode.
///
...
/// matters because a record rust-db writes has to be readable by SQLite and
...
pub fn encode_record_with_limits(
    ...
    if cursor != header_len {
        return Err(DbError::primary(rustdb_base::PrimaryCode::Internal)
...
mod tests {
    ...
    fn an_oversized_field_reports_too_big_rather_than_corruption() {
        ...
        assert_eq!(error.code(), rustdb_base::PrimaryCode::TooBig);
