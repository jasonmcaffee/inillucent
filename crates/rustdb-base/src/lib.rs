...
//! error model shared by every rust-db layer.
//!
...
//!
//! The TDD's crate table names `rustdb-value` and `rustdb-vfs` as the two
//! leaves. Phase 1 needs checked integers, varints, checksums, page arithmetic
//! and the error table *before* values exist, and `rustdb-vfs` needs the same
