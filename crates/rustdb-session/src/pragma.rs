...
use rustdb_base::{error::misuse, DbResult};
use rustdb_sql::catalog_view::{CatalogView, ColumnInfo, IndexOrigin, TableKind};
use rustdb_sql::directive::PragmaArgument;
...
fn expression_text(expr: &rustdb_sql::bind::BoundExpr) -> String {
    use rustdb_sql::bind::BoundExpr;
    match expr {
        ...
        BoundExpr::Unary { op, operand } => match op {
            rustdb_sql::ast::UnaryOp::Negate => format!("-{}", expression_text(operand)),
            rustdb_sql::ast::UnaryOp::Identity => expression_text(operand),
            _ => String::new(),
        },
        _ => String::new(),
    }
...
fn view_columns(
    catalog: &rustdb_catalog::snapshot::CatalogSnapshot,
    table: &rustdb_sql::catalog_view::TableInfo,
) -> Vec<ColumnInfo> {
    let Some(body) = table.view.as_ref() else {
        return Vec::new();
    };
    let authorizer = rustdb_sql::bind::AllowAll;
    let mut binder = rustdb_sql::bind::Binder::new(catalog, &body.ast, &authorizer);
    ...
    bound
        ...
        .map(|(position, column)| {
            ...
            ColumnInfo {
                ...
                affinity: rustdb_value::affinity::for_column(&column.declared_type),
...
pub fn database_list(state: &mut ConnectionState) -> DbResult<PragmaRows> {
    ...
    for (index, schema) in catalog.databases.iter().enumerate() {
        ...
        if index == rustdb_storage::TEMP_DATABASE && !has_temp {
            continue;
        }
        rows.push(vec![
            Value::Integer(seq),
...
fn find_table<'catalog>(
    catalog: &'catalog rustdb_catalog::snapshot::CatalogSnapshot,
    database: Option<usize>,
    folded: &[u8],
) -> Option<&'catalog rustdb_sql::catalog_view::TableInfo> {
    match database {
        Some(index) => {
...
pub fn integrity_check(
    ...
    let limit = argument
        ...
        .unwrap_or(rustdb_storage::check::DEFAULT_PROBLEM_LIMIT as i64);
    let level = if quick {
        rustdb_storage::check::CheckLevel::Quick
    } else {
    ...
    for index in databases {
        let options = rustdb_storage::check::CheckOptions {
            level,
            schema_roots: true,
            extra_roots: Vec::new(),
        ...
        // says is there.
        let Ok(pager) = rustdb_storage::PagerSet::pager(state, index) else {
            continue;
        };
        let Ok(mut report) = rustdb_storage::check::check_database_with_options(pager, &options)
        else {
            continue;
        };
...
) -> std::collections::BTreeMap<u32, rustdb_value::record::KeyInfo> {
...
pub fn refused(name: &[u8], why: &str) -> rustdb_base::DbError {
...
pub fn function_list() -> DbResult<PragmaRows> {
    let mut rows: PragmaRows = Vec::new();
    for entry in rustdb_sql::function::every_function() {
        rows.push(vec![
...
pub fn read(
    ...
    let target = database.unwrap_or(rustdb_storage::MAIN_DATABASE);
    let rows = match name {
        ...
        b"page_size" => {
            let pager = rustdb_storage::PagerSet::pager(state, target)?;
        ...
        b"auto_vacuum" => {
            ...
            vec![vec![Value::Integer(match pager.header().vacuum_mode {
                rustdb_storage::VacuumMode::None => 0,
                rustdb_storage::VacuumMode::Auto => 1,
                rustdb_storage::VacuumMode::Incremental => 2,
            })]]
        }
        b"encoding" => {
            let pager = rustdb_storage::PagerSet::pager(state, target)?;
            let encoding = match pager.text_encoding() {
                rustdb_value::TextEncoding::Utf8 => "UTF-8",
                rustdb_value::TextEncoding::Utf16Le => "UTF-16le",
                rustdb_value::TextEncoding::Utf16Be => "UTF-16be",
            };
...
fn action_name(action: rustdb_sql::ast::ReferentialAction) -> &'static str {
    use rustdb_sql::ast::ReferentialAction;
    match action {
