...
use rustdb_ext::registry::Registry;
use rustdb_ext::vtab::{Context, VirtualCursor, VirtualTable};
use rustdb_sql::catalog_view::{ColumnInfo, TableInfo, TableKind};
use rustdb_sql::vtab::{Declaration, ModuleArguments, ModuleRef, ShadowRoot};
use rustdb_vm::program::VirtualRef;
...
pub fn eponymous_table(registry: &Registry, name: &str) -> DbResult<Option<TableInfo>> {
    ...
    let arguments = ModuleArguments {
        database: rustdb_storage::MAIN_DATABASE,
...
impl rustdb_vm::compile::VirtualPlanner for SessionPlanner {
