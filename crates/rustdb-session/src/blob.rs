...
use rustdb_base::error::{misuse, DbError};
use rustdb_base::ids::PageId;
use rustdb_base::{DbResult, PrimaryCode};
use rustdb_storage::overflow::{self, PayloadPlace};
use rustdb_storage::{BTreeCursor, SeekBias};
...
impl<'a> Blob<'a> {
    ...
    fn with_place<T>(
        &self,
        body: impl FnOnce(&mut rustdb_storage::Pager, &ValuePlace) -> DbResult<T>,
    ) -> DbResult<T> {
...
fn value_place(
    pager: &mut rustdb_storage::Pager,
    place: &PayloadPlace,
    slot: usize,
) -> DbResult<ValuePlace> {
