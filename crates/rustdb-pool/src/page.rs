...
//! moved down a layer, from `rustdb-tree` to `rustdb-pool`, without a line of
//! it changing. `rustdb_tree::page` re-exports it, so every existing caller
...
pub fn page_checksum(page: &[u8]) -> DbResult<u32> {
    ...
    Ok(rustdb_base::checksum::crc32(body))
}
