...
use rustdb_base::checksum::crc32;
use rustdb_base::error::{corrupt, misuse};
use rustdb_base::DbResult;

use crate::PageId;

/// The eight bytes that begin every rust-db data file.
...
impl Meta {
    ...
    pub fn decode(page: &[u8]) -> DbResult<Meta> {
        ...
        if magic != MAGIC {
            return Err(corrupt("the file does not begin with the rust-db magic"));
        }
...
fn checksum(page: &[u8]) -> DbResult<u32> {
    ...
    let tail = page
        .get(at::RESERVED..)
        .ok_or_else(|| corrupt("the meta page is too short to checksum"))?;
    Ok(rustdb_base::checksum::crc32_continue(crc32(head), tail))
}
